...
public sealed class WP213BJourneyTests
{
    ...
    public async Task SwitchPerson_EvidenceCheckedByFirst_RefusedThenSucceedsForTheSecond()
    {
        ...
        try
        {
            ...
            // `WP 21.3B` seeded its in-memory People seam here; at merge the
            // seam sits over the People library (`WP 20.10F`), so the second
            // reviewer is registered, verified and released through the
            // library exactly as a user would add them — the identical path
            // Switch person lists from.
            var people = (IPersonCatalog)host.Services!.GetService(typeof(IPersonCatalog));
            var reviewer = await people.RegisterAsync(
                "second-reviewer-213b",
                new Person { DisplayName = "Second Reviewer", Role = "Checker", IdentityId = secondIdentityId },
                PersonProvenance.Default);
            await host.ReferenceReview!.VerifyAsync(people, reviewer.Id, new ReferenceReviewStatement("WP 21.3B journey"));
            await host.ReferenceReview!.ReleaseAsync(people, reviewer.Id, "WP 21.3B journey");
