...
internal static class InvoicingTestHost
{
    ...
    {
        var childEntries = await Domain(host).Repository.ListChildrenAsync(projectId);
        var children = await Domain(host).Repository.MaterialiseAsync<InvoiceRequest>(childEntries);
