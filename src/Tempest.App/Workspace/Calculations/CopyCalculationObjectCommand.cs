...
/// <see cref="CalculationObjectFactoryRegistry"/>'s own existing Create
/// machinery, mirroring <see cref="Mechanical.CopyMechanicalObjectCommand"/>'s
/// own identical shape; no new Domain capability is introduced. A Calculation
/// Set's own members are copied as-is (frozen at construction, like
/// <c>Configuration.MemberRevisions</c>) — never re-linked to new copies of
/// the members themselves.
/// </summary>
public sealed class CopyCalculationObjectCommand : IWorkspaceCommand
{
    public CopyCalculationObjectCommand(Guid targetObjectId, string targetKind, Guid? newParentId, string? newIdentifier = null, string? newDisplayName = null)
    {
...
/// <summary>Handles <see cref="CopyCalculationObjectCommand"/>.</summary>
public sealed class CopyCalculationObjectCommandHandler : ICommandHandler<CopyCalculationObjectCommand>
{
    ...
    public async Task<CommandResult> HandleAsync(CopyCalculationObjectCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            copy = await _registry.CreateAsync(
                ...
                memberCalculationIds: source is ICalculationSet set ? set.MemberCalculationIds : null,
