...
| `BusinessGovernance` (P07) | Risk, Assets (IP/data), Finance (Assumption/Scenario/Control), Development (Opportunity/Pipeline), Operating. **Returned 2026-09-21 (`ADR-0150`):** Contracts (all seven files, `ContractTemplate` and its catalogue included — `ContractService` issues a contract *from* a Released template, so the folder is one unit), `Pricing.PricingService`, and `DeterminationState.cs` (which `ContractTemplate`/`TemplateDeparture`/`ContractDeliverable` type their review and acceptance states with), with `ContractTests.cs`, the `PricingService` half of `PricingTests.cs` and their `BusinessGovernanceHostRegistrationTests.cs` rows — at the Product Owner's request, so the Tempest-Dashboard export (`contracts.json`/`quotes.json`) carries real figures. Re-pointed at the `Tempest.Core` of that day: no source drift (the moved files compiled unchanged); the returned host-registration test's `PersistenceStore.RootPathConfigurationKey` became `SqlitePersistenceStore.RootPathConfigurationKey` (`WP 18.1A`). | `Money`, `CurrencyCode`, `EffectivePeriod`, `RateCard`, `RateCardCatalog`, and the namespace-root `BusinessAuthority.cs`/`Confidentiality.cs`/`BusinessEvidence.cs`/`BusinessStewardship.cs`/`BusinessGovernanceValidation.cs` they depend on directly; from 2026-09-21 also everything under Contracts/, `Pricing/PricingService.cs`, `DeterminationState.cs`, and the new `Quotations/` (`ADR-0150`, never frozen) | `ADR-0129` — amended, subject partly kept; `ADR-0130` — kept, subject whole; `ADR-0150` — Contracts and `PricingService` returned | 7,527 / 2,383 moved by `WP 18.0C`; 2,047 / 612 returned by `ADR-0150` |
