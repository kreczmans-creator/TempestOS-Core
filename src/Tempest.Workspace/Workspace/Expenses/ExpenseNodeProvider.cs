...
public sealed class ExpenseNodeProvider : IProjectExplorerNodeProvider
{
    ...
    private async Task<List<IEngineeringObject>> LiveProjectsAsync(CancellationToken cancellationToken)
    {
        var entries = (await _context.Repository.ListByKindAsync(MechanicalObjectFactoryRegistry.Project, cancellationToken).ConfigureAwait(false))
            ...
            .ToList();
        return (await _context.Repository.MaterialiseAsync<IEngineeringObject>(entries, cancellationToken).ConfigureAwait(false))
            .Where(IsLive)
            .ToList();
    }
    ...
    private async Task<List<ProjectExpense>> LiveExpensesUnderProjectAsync(Guid projectId, CancellationToken cancellationToken)
    {
        var entries = (await _context.Repository.ListChildrenAsync(projectId, cancellationToken).ConfigureAwait(false))
            .Where(entry => !entry.IsDeleted)
            .ToList();
        return (await _context.Repository.MaterialiseAsync<ProjectExpense>(entries, cancellationToken).ConfigureAwait(false))
            .Where(IsLive)
            .ToList();
    }
