...
/// optional, plugin-unaware <c>componentScopeProvider</c> hook, keyed by the hosted
/// service's own <see cref="Type"/>, so that <c>TempestHost</c> could push a plugin's
/// own component principal around each start/stop call. Nothing third-party ever
/// loaded; the hook and the trust platform behind it are frozen at
