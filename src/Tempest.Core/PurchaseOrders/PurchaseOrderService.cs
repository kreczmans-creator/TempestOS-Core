...
public sealed class PurchaseOrderService : IPurchaseOrderService
{
    ...
    {
        var existing = await _context.Repository.ListByKindAsync(PurchaseOrder.CanonicalKind, cancellationToken).ConfigureAwait(false);
        var fullPrefix = $"{ReferencePrefix}{year.ToString(CultureInfo.InvariantCulture)}-";

        var max = 0;
        foreach (var candidate in existing.OfType<PurchaseOrder>())
        {
