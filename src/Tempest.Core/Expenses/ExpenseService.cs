...
public sealed class ExpenseService : IExpenseService
{
    ...
    {
        // `WP 21.5B`: list results are index rows — materialise the expenses.
        var childEntries = await _context.Repository.ListChildrenAsync(projectId, cancellationToken).ConfigureAwait(false);
        var children = await _context.Repository.MaterialiseAsync<ProjectExpense>(childEntries, cancellationToken).ConfigureAwait(false);
