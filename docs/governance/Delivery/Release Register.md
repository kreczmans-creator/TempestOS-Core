...
| v0.5.0 | **In progress, not yet tagged** | `docs/releases/v0.5.0/` (ReleasePlan, WorkPackages); root `VERSION` file still reads `0.4.0` | **"Developer Experience"** — renumbers `v0.4.0`'s own deferred `WP 4.6A`–`WP 4.9` as `WP 5.0A`–`WP 5.3` (see `ReleasePlan.md`'s "A Note on Renumbering"), and grew a new `WP 5.0C`/`WP 5.0D` pair beyond that original scope once `WP 5.0C`'s own Repository Investigation confirmed `Tempest.App` still had no composition root. `WP 5.0A` (Navigation Framework Architecture), `WP 5.0B` (Navigation Framework Implementation), and `WP 5.0C` (Shell & Composition Framework Architecture) all complete — `ADR-0031`–`ADR-0035`, `Navigation Framework Architecture.md`, `Shell & Composition Framework Architecture.md`, `Tempest.Core.Navigation`, 45 new tests (400 total); `WP 5.0D` onward not started. |
