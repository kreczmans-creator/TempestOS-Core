...
| v0.4.0 | **In progress, not yet tagged** | `docs/releases/v0.4.0/` (ReleasePlan, Architecture, WorkPackages, CHANGELOG, Risks, Testing, ReleaseChecklist); root `VERSION` file still reads `0.3.0` | "Platform Services" milestone — Platform Contracts, Module SDK, Plugin Manifest, Sample Module, Event Bus, Background Services, the Governance Register Baseline, and the Foundation phase closeout (`WP 4.5B`) complete; Navigation, Command Framework, Diagnostics, Developer Experience remain (see `Feature Register.md`). The Foundation phase within this release is now formally closed — see `docs/releases/Platform Foundation Completion Report.md`. |
