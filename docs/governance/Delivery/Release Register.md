...
| v0.5.0 | **In progress, not yet tagged** | `docs/releases/v0.5.0/` (ReleasePlan, WorkPackages); root `VERSION` file still reads `0.4.0` | **"Developer Experience"** — renumbers `v0.4.0`'s own deferred `WP 4.6A`–`WP 4.9` as `WP 5.0A`–`WP 5.3` (see `ReleasePlan.md`'s "A Note on Renumbering"). `WP 5.0A` (Navigation Framework Architecture) and `WP 5.0B` (Navigation Framework Implementation) both complete — `ADR-0031`, `ADR-0032`, `Navigation Framework Architecture.md`, `Tempest.Core.Navigation`, 45 new tests (400 total); `WP 5.1` onward not started. |
