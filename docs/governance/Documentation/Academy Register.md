...
## 03 Work Packages (56 retrospectives)
...
Architecture) + 61 (Work Packages) + 5 (Design Patterns) + 5 (Case
Studies) + 5 (Engineering Standards) + 4 (top-level meta) = 109 files
...
`WP 8.0B`, `WP 8.1A`), not just the grand total, continuing the standing
practice `WP 5.4`'s own repository review recommended. `02 Runtime
Architecture/17-engineering-workspace.md` (introduced by `WP 8.0A`) was
updated in place twice — by `WP 8.0B`, then by `WP 8.1A` — neither
