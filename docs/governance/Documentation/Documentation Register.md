...
| `docs/releases/v0.7.0/` | The in-progress ("Engineering Foundation") v0.7.0 release's own prepared-in-advance skeleton documents (`WorkPackages.md`, `ReleaseNotes.md`, `Retrospective.md`), `WP 7.0A`'s five, `WP 7.0B`'s eight, and `WP 7.0C`'s eight planning/review deliverables, plus `WP 7.1A`'s seven and `WP 7.1B`'s seven implementation deliverables — the first two real Work Package implementations of this release | `Release Register.md`; `Future Capability Register.md` |
