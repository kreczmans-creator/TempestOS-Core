...
| `docs/releases/v0.7.0/` | The ("Engineering Foundation") v0.7.0 release's own prepared-in-advance skeleton documents (`WorkPackages.md`, `ReleaseNotes.md`, `Retrospective.md`), `WP 7.0A`'s five, `WP 7.0B`'s eight, and `WP 7.0C`'s eight planning/review deliverables, plus `WP 7.1A`'s seven, `WP 7.1B`'s seven, `WP 7.1C`'s seven, `WP 7.1D`'s eight, and `WP 7.1E`'s eight implementation deliverables (the last two each including a dedicated Security Review Report) — five real Work Package implementations, completing the entire Engineering Foundation programme — plus `WP 7.1F`'s ten closing-certification deliverables (`Engineering Core Certification Report.md` and nine companions), and `ENGINEERING_CORE_COMPLETION_REPORT.md` at the repository root, the programme's own permanent historical milestone document | `Release Register.md`; `Future Capability Register.md` |
