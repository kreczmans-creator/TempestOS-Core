...
| **Status** | First concrete step implemented — `WP 14.0A` shipped the TempestOS Companion (a separate-process Avalonia single-view mobile client, `ADR-0113`) with read-only snapshot caching and explicit freshness states (`ADR-0115`) over a new REST query-and-action surface (`ADR-0114`). Deliberately still NOT full offline synchronisation: no offline writes (`AT-24`), no sync protocol, no mobile platform heads yet (`TD-57`), off-box reach gated on `FCR-0003`/`FCR-0004` (`TD-58`). |
| **Priority** | Medium — raised from Low by `WP 14.0A`: a real client now exists to serve |
| **Business Value** | A mobile operational window into a running TempestOS — awareness, triage, and controlled quick actions away from the desk |
| **Engineering Effort** | Remaining: High — platform heads, real auth/TLS, and genuine synchronisation semantics |
| **Dependencies** | Remaining scope depends on `FCR-0022` (cloud synchronisation), `FCR-0003` (authentication), `FCR-0004` (TLS) |
| **Proposed Target Release** | Companion baseline: `v0.14.0` (in preparation). Full offline synchronisation: not yet scheduled |
| **Related ADRs** | ADR-0113, ADR-0114, ADR-0115 |
| **Related Work Packages** | `WP 14.0A` |
| **Academy Impact** | `WP14.0A-tempestos-companion-mobile-companion-application.md` (shipped with the Work Package) |
| **Notes** | Sourced from `Security Roadmap.md` item 9. `WP 14.0A` update: pulled forward from `Product Roadmap.md` Phase 8 by that Work Package's own explicit commissioning brief — the deviation is disclosed in `ADR-0113`'s Status and the retrospective, not silent. |

#### FCR-0090 — Companion Push Notifications

| Field | Value |
|---|---|
| **Category** | Infrastructure |
| **Description** | Real push delivery of platform notifications to the Companion (and any future mobile head), replacing `WP 14.0A`'s poll-only model — today the Companion reads a bounded, in-memory window over `IPlatformNotification` events (`CompanionNotificationBuffer`, 50 entries, reset at Host restart) via a query route; nothing reaches a device the moment something happens. |
| **Status** | Identified, not started |
| **Priority** | Low — meaningful only once the Companion runs on real devices off-box |
| **Business Value** | Would make the Companion's attention loop proactive (an approval request or health degradation reaches the engineer's pocket) rather than poll-driven |
| **Engineering Effort** | High — requires durable notification persistence (ADR-0046 deliberately built none), a delivery channel, and per-device privacy decisions (what may appear on a lock screen) |
| **Dependencies** | `FCR-0003`/`FCR-0004` (a device off-box must authenticate securely first, `TD-58`); ADR-0046's derived-notification model as the source |
| **Proposed Target Release** | Not yet scheduled |
| **Related ADRs** | ADR-0046, ADR-0113, ADR-0114 |
| **Related Work Packages** | `WP 14.0A` (the poll-based baseline and this entry's source) |
| **Academy Impact** | Would extend the `WP 14.0A` retrospective's notification section once designed |
| **Notes** | Sourced from `WP 14.0A`'s own disclosed scope decision (poll-only notification awareness) and its Companion Security Review's lock-screen-privacy note. |

#### FCR-0091 — Offline Dashboard Display Client

| Field | Value |
|---|---|
| **Category** | Infrastructure |
| **Description** | A dedicated always-visible operational display (office/workshop wall screen) rendering passive TempestOS awareness — project health, attention items, milestones, activity — consuming the identical Companion query surface (`ADR-0114`) rather than any display-specific data path, so Cockpit → Companion → Dashboard stay one information architecture. |
| **Status** | Identified, not started |
| **Priority** | Low — no such display exists or has been requested by a named deployment |
| **Business Value** | Passive, ambient visibility of platform state for a co-located team |
| **Engineering Effort** | Low-Medium — a read-only Avalonia surface over the already-shipped query routes |
| **Dependencies** | `ADR-0114`'s query surface (shipped, `WP 14.0A`); off-box placement gated on `FCR-0003`/`FCR-0004` exactly as `TD-58` records for the Companion |
| **Proposed Target Release** | Not yet scheduled |
| **Related ADRs** | ADR-0113, ADR-0114 |
| **Related Work Packages** | `WP 14.0A` |
| **Academy Impact** | None until designed |
| **Notes** | Sourced from `WP 14.0A`'s commissioning brief, which described an "existing offline dashboard" — repository reconnaissance verified no such implementation or design exists anywhere in code or governance, so the concept enters the record here, as future capability, rather than being retrofitted as though it had existed. Marked **Inferred** from the brief, not from any repository artifact. |
...
#### FCR-0089 — Business Management Awareness (Invoices, Contracts, Commercial Deadlines)

| Field | Value |
|---|---|
| **Category** | Commercial |
| **Description** | Business-management awareness surfaces — invoices due/overdue, contracts outstanding, commercial deadlines — for the Companion's "Business Pulse", the future offline dashboard (`FCR-0091`), and the desktop Cockpit. No such domain exists anywhere in TempestOS today: the only trace is the bootstrap-era, dead `ProjectModel.Customer`/`ContractNumber` fields (`FCR-0026`'s own identical evidence), and the platform is deliberately not an accounting system (`ADR-0050`: licensing/billing policy stays outside the platform). |
| **Status** | Identified, not started |
| **Priority** | Low — no business-management data model, integration, or named user need exists |
| **Business Value** | Would let a practice see commercial state (what is due, what is outstanding) beside engineering state in one operational picture |
| **Engineering Effort** | High — a genuine domain family (or an integration boundary to an authoritative external business system) plus its Workspace, Companion, and API presence |
| **Dependencies** | A decision whether TempestOS owns this data or integrates with an authoritative external system — undecided; `ADR-0114`'s query surface would carry the read side either way |
| **Proposed Target Release** | Not yet scheduled |
| **Related ADRs** | ADR-0113 (records the Companion shipping without this, honestly), ADR-0114 |
| **Related Work Packages** | `WP 14.0A` (this entry's source) |
| **Academy Impact** | None until designed |
| **Notes** | Sourced from `WP 14.0A`'s commissioning brief, which asked for business awareness "where supported by the existing model" — reconnaissance verified it is not supported (zero invoice/contract entities in `src/`), so the Companion ships engineering awareness plus Milestone deadlines (`IMilestone.TargetDate`, the Domain's one real deadline carrier) and this capability enters the register instead of being faked. Marked **Inferred** from the brief. |
...
#### FCR-0092 — Desktop Workspace Alignment to the Tempest Engineering Design System

| Field | Value |
|---|---|
| **Category** | Workspace |
| **Description** | Realign `Tempest.Desktop`'s visual layer (FluentTheme defaults, `ApplicationPalette`'s five provisional brushes, `HealthColors`/`SeverityColors`' named .NET colours, emoji glyphs in the Cockpit/QAT) to the authoritative Tempest Engineering Design System `WP 14.1A` applied to the Companion: the navy instrument/paper themes, the indigo/cyan/violet triad, machine-state green/amber/red, squared corners, UPPERCASE Chakra labels, mono machine data, and the supplied mark/lockups. |
| **Status** | Identified, not started |
| **Priority** | Medium — the desktop and the Companion currently read as different products side by side, exactly the inconsistency the brand exists to prevent |
| **Business Value** | One recognisable product family across desktop, Companion, and any future display client |
| **Engineering Effort** | Medium — the desktop's theming is already centralised (`ApplicationPalette`/`DesignTokens`/`HealthColors`, `WP 10.5A`), so this is a values-and-idiom pass, not an architecture change; the emoji-glyph removals touch more files |
| **Dependencies** | The design system pack (received `WP 14.1A`; condensed as `docs/design/Tempest Engineering Design System Reference.md`); `WP 14.1A`'s Companion realisation as the reference implementation |
| **Proposed Target Release** | Not yet scheduled — deliberately not done as a `WP 14.1A` side effect (scope discipline: that Work Package's subject was the Companion) |
| **Related ADRs** | ADR-0094 (the theming mechanism this would re-value), ADR-0113 (corrected brand realisation) |
| **Related Work Packages** | `WP 14.1A` (this entry's source), `WP 10.5A` (the theming architecture it would flow through) |
| **Academy Impact** | Would update `21-engineering-cockpit-graphical-implementation.md` and the `WP 10.5A` retrospective's theming sections |
| **Notes** | Sourced from `WP 14.1A`'s own gap analysis: the Companion now carries the authoritative brand while the desktop retains `WP 10.x` provisional styling — disclosed there and registered here rather than silently left divergent. |
