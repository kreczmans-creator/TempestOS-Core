...
| **Notes** | Inferred purely from `Capability Categories.md`'s own Engineering Discipline list, the same basis as `FCR-0030`/`FCR-0031` — no prior document named this capability directly at planning time. Implemented `WP 7.1D` extended substantially beyond `WP7.0C Engineering Foundation Contracts.md`'s own illustrative shape (metadata, assumptions, constraints, a validation model, an execution context, material references) to satisfy this Work Package's own "engineering evidence, not merely a numerical answer" requirement — resolved in `ADR-0056`, not an unauthorised deviation. First Engineering Foundation Work Package to include a dedicated Security Review. **Delivered by, `WP 21.9.1` note, 2026-09-15:** this framework is used substantively by discipline-facing calculation modules for the first time in `release/v0.21.0` — `WP 21.7A`'s eleven engineering calculation modules (`Tempest.Core.Calculations.Modules`) and the `WP 21.7B`/`21.7C` Engineering Calculators surface — nine releases after this entry was first marked Implemented, on the Product Owner's direct instruction to build them "like RoyMech's online calculators … no code" (`docs/releases/v0.21.0/Execution Plan.md`). |
...
| **Notes** | Raised directly by `WP9.2A Future Capability Assessment.md` and `WP9.2A Technical Debt Assessment.md` (`TD-30`). **Reaffirmed, not implemented, `WP 21.9.1` note, 2026-09-15:** `WP 21.3A` (`TD-30`) considered this entry directly and retired `ICalculationResult`/`IVerificationResult` with reasons recorded rather than implementing them — turning an immutable evidentiary snapshot into a live, addressable `IEngineeringObject` remains "a real Domain design question, not a mechanical add," this entry's own words, quoted back by `WP 21.3A`. Status unchanged. |
...
| **Academy Impact** | Would extend `WP9.2A-engineering-calculations-workspace.md` once implemented |
| **Notes** | Raised directly by `WP9.2A Future Capability Assessment.md` and `WP9.2A Technical Debt Assessment.md` (`TD-30`). **Reaffirmed, not implemented, `WP 21.9.1` note, 2026-09-15:** `WP 21.3A` (`TD-30`) considered this entry directly too and retired `IApprovalGate` with the reason recorded — it is "a workflow-gate concept the product's own 'no workflow engine' guard and `ADR-0087`/`ADR-0090` already rule out," `WP 21.3A`'s own words. Status unchanged. |
...
| **Status** | **Implemented (`WP 21.3A`, `TD-29`, 2026-09-15)** — `CalculationRecord<TResult>` retains the input it ran with (`Input`/`InputTypeName`); `ICalculationEngine.ReRunAsync` runs the identical retained input, or a supplied changed one, exactly the Domain-layer option this entry named, extending the stored shape rather than adding a session-scoped cache |
...
| **Engineering Effort** | Delivered — `WP 21.3A`, part of a five-day package alongside `TD-22`/`TD-30` |
| **Dependencies** | `CalculationRecordDto<TResult>` (Implemented, `WP 7.1D`) would need its own extension for the Domain-layer option |
| **Proposed Target Release** | Shipped, `release/v0.21.0` (release candidate, not yet tagged `v1.0.0`) |
| **Related ADRs** | None yet |
| **Related Work Packages** | `WP 7.1D` (defined the current, input-free stored shape); `WP 9.2A` (disclosed the gap via `TD-29`); `WP 21.3A` (implemented) |
| **Academy Impact** | Would extend `WP9.2A-engineering-calculations-workspace.md` once implemented |
| **Notes** | Raised directly by `WP9.2A Future Capability Assessment.md` and `WP9.2A Technical Debt Assessment.md` (`TD-29`). **Delivered, `WP 21.9.1` note, 2026-09-15:** closed by `WP 21.3A`, surfaced as the `calculations.rerun` and `calculations.compare-with-previous` commands and, completed further by `WP 21.7C`, offered directly on the Engineering Calculators result pane. See `docs/releases/v0.21.0/Release Notes.md`. |
