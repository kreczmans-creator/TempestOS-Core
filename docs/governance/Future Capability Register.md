...
| **Status** | **Design complete — `WP 13.0A`** (`ADR-0110`–`ADR-0112`, `Plugin Trust & Isolation Architecture.md`); implementation pending `WP 13.0B` |
...
| **Proposed Target Release** | `v0.13.0` — architecture complete (`WP 13.0A`); implementation `WP 13.0B`. No longer gated on `FCR-0002`'s own schedule; the Product Owner's commitment fired both together. |
| **Related ADRs** | ADR-0044, ADR-0110, ADR-0111, ADR-0112 |
| **Related Work Packages** | `WP 5.0S` (found), `WP 5.1A` (widened scope), `WP 6.1` (built the mechanism), `WP 6.8` (recommended for `v0.7.0`), `WP 13.0A` (designed the retrofit), `WP 13.0B` (implementation, not yet started) |
...
| **Dependencies** | `FCR-0001` — architecture now complete (`WP 13.0A`); implementation (`WP 13.0B`) must land first or alongside a real plugin, per that Work Package's own dependency, unchanged |
...
| **Status** | **Architecture designed — `WP 13.0A`** (`Runtime:Plugins:RootDirectory`/`Runtime:Plugins:ManifestFileName` configuration keys, `Plugin Platform Architecture.md`); not yet implemented |
...
| **Engineering Effort** | Low — confirmed at the architecture stage: both keys are optional, default to the existing fixed conventions, and are consulted at the seam `ADR-0026` already anticipated, requiring no Host Lifecycle phase change |
| **Dependencies** | None |
| **Proposed Target Release** | `v0.13.0` — architecture complete (`WP 13.0A`); implementation `WP 13.0B` |
...
| **Related Work Packages** | `WP 4.2`, `WP 13.0A` |
| **Academy Impact** | None until resolved |
