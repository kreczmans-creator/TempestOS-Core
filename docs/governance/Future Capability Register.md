...
| **Status** | Identified — **architecture complete (`WP 7.2B`)**, not yet Implemented. `WP7.2B Requirements Platform Architecture.md` and ten companion deliverables design the complete platform (classification, domain model, digital thread, dependency analysis, security architecture, standards mapping); zero production code written. `ADR-0013` classification decided (Platform Service, `WP7.2B Requirements Platform Architecture.md` §2), not yet ratified as a written ADR — reserved as `ADR-0058`. |
| **Priority** | High — the only Engineering Discipline category with both a completed technical foundation (`FCR-0029`, `FCR-0033`, both Implemented) and a named platform-level hook (`ADR-0013`), and now the only one with a complete architecture as well |
...
| **Engineering Effort** | Unknown — architecture complete, but no implementation estimate is meaningful until `WP7.2B Required ADR Catalogue.md`'s own three reserved decisions (`ADR-0058`–`ADR-0060`) are ratified; `WP7.0B Candidate Work Package Catalogue.md`'s own Candidate `I` disclosed this as "the least architecturally grounded candidate" prior to `WP 7.2B`'s own review |
| **Dependencies** | `ADR-0013` classification decided at architecture level (Platform Service); both technical dependencies satisfied — `FCR-0029` (Engineering Data Model) and `FCR-0033` (Verification Framework) are both **Implemented** and certified |
| **Proposed Target Release** | `v0.8.0` (recommended, not committed — `WP7.2A Recommended Programme.md`) |
| **Related ADRs** | ADR-0013 (names this capability directly in its own Future Considerations); `ADR-0058`–`ADR-0060` reserved, not yet written (`WP7.2B Required ADR Catalogue.md`) |
| **Related Work Packages** | `WP 7.2A` (recommended this capability as the next programme); `WP 7.2B` (designed its complete architecture) |
| **Academy Impact** | `WP7.2B Academy Plan.md` recommends a new concept guide once implementation exists to derive worked examples from; no concept guide written yet (architecture only) |
| **Notes** | Sourced from `PROJECT_STATUS.md`'s own Long-Term Vision section; `ADR-0013`'s own Future Considerations; `Threat Model.md` assumption 1 ("requirements, analysis, verification records"); `WP7.1E Future Capability Recommendations.md` Recommendation 1 (recommends this capability consume `IVerificationService` directly — confirmed and detailed by `WP7.2B Platform Integration Report.md` §1); `WP7.2A Strategic Roadmap Review.md`, `WP7.2A Programme Comparison Matrix.md`, `WP7.2A Recommended Programme.md`; `WP7.2B Requirements Platform Architecture.md` and its ten companion deliverables. |
