...
| **Status** | Identified — **recommended by `WP 7.2A`** as the next implementation programme's own scope (`WP7.2A Recommended Programme.md`, scored 46/55, the highest of seven candidates evaluated); not yet approved — recommendation is not approval |
| **Priority** | High — the only Engineering Discipline category with both a completed technical foundation (`FCR-0029`, `FCR-0033`, both Implemented) and a named platform-level hook (`ADR-0013`) |
...
| **Engineering Effort** | Unknown — requires its own Architecture Work Package before any estimate is meaningful; `WP7.0B Candidate Work Package Catalogue.md`'s own Candidate `I` (carried forward as `WP7.2A Candidate Work Package Catalogue.md`'s own Candidate `K`) disclosed this as "the least architecturally grounded candidate" evaluated to date |
| **Dependencies** | Requires its own explicit Platform-Service-vs-Module classification decision (`ADR-0013`'s own Future Considerations name this exact capability as an example) before design begins; both technical dependencies now satisfied — `FCR-0029` (Engineering Data Model) and `FCR-0033` (Verification Framework) are both **Implemented** and certified, not merely identified |
...
| **Related Work Packages** | `WP 7.2A` (recommended this capability as the next programme) |
| **Academy Impact** | Would be the first Academy content for the Systems Engineering category once designed |
| **Notes** | Sourced from `PROJECT_STATUS.md`'s own Long-Term Vision section; `ADR-0013`'s own Future Considerations; `Threat Model.md` assumption 1 ("requirements, analysis, verification records"); `WP7.1E Future Capability Recommendations.md` Recommendation 1 (recommends this capability consume `IVerificationService` directly); `WP7.2A Strategic Roadmap Review.md`, `WP7.2A Programme Comparison Matrix.md`, `WP7.2A Recommended Programme.md`. |
