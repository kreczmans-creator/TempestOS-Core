...
| **Last Reviewed** | 2026-07-27 (WP 5.0D) — counts re-verified directly via a fresh `dotnet test` run; 446/446, 46 new tests added (new `Shell/` directory, plus 8 new tests in `Runtime/`). |
...
**Total: 60 test files, 420 `[Fact]`/`[Theory]` attribute occurrences.**

## Reconciling Attribute Count Against Executed Test Count

`dotnet test` reports **446** executed tests, 26 more than the 420 raw
...
| **Current (WP 5.0D)** | **446** — re-verified directly, 0 failures |
...
The 446 figure above is cross-checked directly against
`Validation Register.md`'s own Test Gate row (also 446, from the same
