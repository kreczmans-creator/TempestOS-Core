...
| **Last Reviewed** | 2026-07-28 (WP 5.1A, Command Framework Architecture) — one new debt item disclosed (TD-11, registration-order squatting, affecting both Navigation and the newly-designed Command Framework); TD-09's own scope widened to explicitly name the Command Framework as a second affected surface. |
| **Related Documents** | `docs/releases/v0.4.0/Risks.md`; `Validation Register.md`; `Hosted Services Register.md`; `Plugin Register.md`; `docs/security/Platform Security Review v0.5.0.md`; `docs/security/Security Roadmap.md`; `docs/architecture/Command Framework Architecture.md`. |
| **Related ADRs** | ADR-0009, ADR-0021, ADR-0024, ADR-0025, ADR-0026, ADR-0028, ADR-0029, ADR-0032, ADR-0037. |
...
| TD-11 | Command and Navigation registration-order squatting: "first registration wins" rejects a *later* duplicate but does not establish the *first* registrant was the intended owner of a well-known Id — a plugin whose module Id sorts earlier (by `ModuleLifecycleManager`'s existing ascending-Id initialisation order) can legitimately claim a well-known command or navigation Id before its real owner registers | Pre-existing for Navigation since WP 5.0B/ADR-0032 (undetected until now); newly designed-in for Commands, WP 5.1A; both disclosed together by WP 5.1A's own Security Review (Finding CMD-1) | None named yet — trigger: paired with TD-09's resolution (an ownership/priority/reservation model) | Open — see `docs/architecture/Command Framework Architecture.md` Finding CMD-1, `docs/security/Security Roadmap.md` item 10 |

**Total: 11 tracked debt items — 2 Resolved, 1 Partially resolved, 8 Open.**
