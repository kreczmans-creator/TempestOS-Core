...
**Total: 16 namespaces (15 declared + the global namespace) across 3
projects, 116 `.cs` files under `src/` excluding generated `obj`/`bin`
artifacts (Verified by direct count; adds `Tempest.Core.Navigation`'s 7
files and 3 new `Tempest.Samples` files, `WP 5.0B`).**
