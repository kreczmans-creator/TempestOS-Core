...
| **Last Reviewed** | 2026-07-29 (WP 6.7, Export/Import) — added `Tempest.Core.ExportImport`; every row's own file count re-derived directly again (`grep -rl "^namespace X;"`), consistent with `WP 6.0`'s/`WP 6.2`'s/`WP 6.3`'s own prior passes. |
...
| **Related ADRs** | ADR-0016, ADR-0024, ADR-0036, ADR-0037, ADR-0038, ADR-0039, ADR-0040, ADR-0046, ADR-0047, ADR-0048, ADR-0049, ADR-0051. |
...
**Total: 28 namespaces (27 declared + the global namespace) across 3
...
`.cs` file is still part of the `src/` file total below. 259 `.cs` files
under `src/` excluding generated `obj`/`bin` artifacts (258 across the
3 in-scope projects + 1 in `Tempest.Templates.Module`) — re-derived
directly by `WP 6.7`, per-namespace, via `grep -rl "^namespace X;"
src/` for every row above, not incremented from the prior figure. `WP
6.7` itself adds the new `Tempest.Core.ExportImport` namespace (16
files) and 7 new `Tempest.Samples` files (`ExportImportSampleModule.cs`,
`SettingExportImportAdapter.cs`, `SampleExportArtifactStore.cs`,
`ExportSampleDataCommand.cs`, `ExportSampleDataCommandHandler.cs`,
`ImportSampleDataCommand.cs`, `ImportSampleDataCommandHandler.cs`).**
