...
| `Tempest.Core.ReferenceData` | Tempest.Core | 31 | The infrastructure every `Group A` reference library shares: `IReferenceDataCatalog<T>`/`ReferenceDataCatalog<T>`, `IReferenceRecord<T>`/`ReferenceRecord<T>`, `ReferenceProvenance` and its extraction/verification enums, `ReferenceValidationState`/`ReferenceValidationStates`, `IReferenceValidationService<T>`/`ReferenceValidationService<T>`/`ReferenceValidationRules`, `ReferenceValue<T>`/`ReferenceRange<T>`/`ReferenceQuantityValue`/`ReferenceQuantityCodec`, `ReferenceComparer`/`ReferenceComparison`, `StandardReference`, the `IStandardResolver` and `IReleasedConstantSource` cross-library seams, `ReferenceDataException` and six subtypes | Group A, `ADR-0126` |
