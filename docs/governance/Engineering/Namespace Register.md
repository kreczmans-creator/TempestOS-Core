...
| **Coverage Status** | **Complete for declared scope, namespace and file counts both re-derived `WP 16.4B-R6` (2026-09-06).** **47 of 47 in-use namespaces** under `src/Tempest.Core/`, `src/Tempest.App/`, `src/Samples/Tempest.Samples/`, `src/Validation/Tempest.Validation/` are listed, zero omitted, across **728 `.cs` files** — 725 files carrying a namespace declaration plus 3 global-namespace files (`src/Tempest.Core/AssemblyInfo.cs`, `src/Tempest.App/AssemblyInfo.cs`, `src/Tempest.App/Program.cs`). Both figures agree with the Total line below the Entries table and with every per-namespace row, checked name by name. **Correction, `WP 16.4B-R6`:** this field previously read "46 … 46 of 46 … (724 files)". `WP 16.4B-R5` rewrote it and announced it as a re-derivation, but the figures it produced matched neither the source tree (47 / 728) nor this register's own Total line seventy-odd lines below, which already said 47 / 725 + 3 = 728. Re-derived here with the register's own declared methodology — BOM stripped, `bin/`/`obj/` pruned, first `namespace` declaration per file — over the four in-scope roots only: `find src/Tempest.Core src/Tempest.App src/Samples/Tempest.Samples src/Validation/Tempest.Validation -name '*.cs' -not -path '*/bin/*' -not -path '*/obj/*'` → 728, and the same list piped through `sed -e '1s/^\xEF\xBB\xBF//' \| grep -oP '^\s*namespace\s+\K[\w.]+' \| head -1 \| sort \| uniq -c` → 47 namespaces over 725 files. Found by the fifth `v0.16.0` review board (`P4-02`). `src/Tempest.Desktop/` (**89 files, 14 namespaces**, re-derived here — this field also carried the "88 files, 9 namespaces" figure the disclosure below the Entries table had already corrected) remains explicitly out of scope, per the Scope field and that disclosure. |
