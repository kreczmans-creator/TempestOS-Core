...
| `SupersededEngineeringObjectException` | `EngineeringDomainException` | Engineering Domain | Application logic's own error (not Host-level); thrown by `EngineeringObjectBase.PersistStateAsync` when a durable write is attempted through an instance that `ReviseAsync` has already superseded. Refusing the write is the point: allowing it silently discarded whatever it wrote, and where the lost field was an attachment reference the reconciliation sweep then deleted the file's bytes as an orphan — added `WP 16.4B-R4`, 2026-09-05 |
...
`awk -F'|' 'NR>=25 && NR<=114 && /^\| \`/ {c=$4; gsub(/^ +| +$/,"",c); print c}' … | sort | uniq -c`;
every category matched except Engineering Domain).
**Correction, `WP 16.4B-R6` (2026-09-06):** this table stated
Engineering Domain 9 and totalled 89 while the Entries table held 10
Engineering Domain rows and 90 rows overall.
`SupersededEngineeringObjectException` was added to the Entries table by
`WP 16.4B-R4` and never counted here; `WP 16.4B-R5` corrected the
**Coverage Status** and the **Total** statements of the same number and
left this third statement of it wrong. Engineering Domain 9 → 10, total
89 → 90. Found by the fifth `v0.16.0` review board (`P4-01`).
Note that the arithmetic as previously written was internally correct —
it correctly summed a table that was short by one row-count — which is
why neither the sum nor Governance Health Check Check 10 (which compares
Entries-table row names to source class names only, and never parses
this table) could detect it. **Correction, `WP 16.4B-R1`:** this table had not been
