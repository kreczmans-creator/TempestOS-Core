...
| `BearingReference` | `Tempest.Core.Bearings.BearingCatalog.BearingDocumentKind` | A bearing reference-data record — the engineering description of one bearing, as a source stated it. |
| `StandardReference` | `Tempest.Core.Standards.StandardCatalog.StandardDocumentKind` | A standards-register entry — the bibliographic identity and publisher status of one standard, never its content. |
| `FastenerReference` | `Tempest.Core.Fasteners.FastenerCatalog.FastenerDocumentKind` | A fastener reference-data record — the engineering description of one fastener, as a source stated it. |
| `ComponentReference` | `Tempest.Core.Components.ComponentCatalog.ComponentDocumentKind` | A mechanical-component reference-data record — one spring, gear, drive element or standard component, as a source stated it. |
| `EngineeringConstant` | `Tempest.Core.Constants.ConstantCatalog.ConstantDocumentKind` | An engineering-constant record — one constant's dimensioned value, its uncertainty and where it applies. |
| `ManufacturingProcessReference` | `Tempest.Core.Manufacturing.ProcessCatalog.ProcessDocumentKind` | A manufacturing-process reference-data record — what sources published about one process. Deliberately distinct from `ManufacturingOperation`, which is an operation performed on a real part. |
...
| `supersedes` | `Tempest.Core.EngineeringDomain.GovernanceRelationshipKinds.Supersedes` | An object to the object it replaced. Written by `Decision.SupersedesAsync` and, since `A4`, by `BearingCatalog.SupersedeAsync` — two write sites, one declaring class, one direction (the superseder links to what it supersedes). **Corrected `A4` (2026-09-05):** this row read "**Undeclared** — conventional only, no confirmed write site", which was already wrong when written — `GovernanceRelationshipKinds.Supersedes` and `Decision.SupersedesAsync` both predate it. `A4` found the drift while checking whether a bearing-specific supersession kind was needed; it was not, precisely because this one already exists. |
