...
| **Coverage Status** | **STALE AND DISCLOSED, NOT FIXED — read at the `v0.16.0` pre-release integration build (2026-09-07).** This field and the Total line below both say 41 entries; the Entries table below holds more than that; and `docs/architecture/Platform Service Map.md` — the Source of Truth this register declares — holds a third, different number. The drift entered on the Foundation line and was carried through the merge unreconciled. Not corrected here for the same reason as the Dependency Injection Register: reconciling a register against a Source of Truth that itself disagrees is a Work Package, not a merge reconciliation. Nothing machine-checks this register (`TD-123`); this recurrence is `TD-153`. *Superseded narrative:* **Complete, re-verified `A4` (2026-09-05).** 41 of 41 known Platform Services are listed — the two `A4` added (Bearing Library, Bearing Validation) plus — the 3 `v0.13.0`/`v0.14.0` services backfilled by `WP 16.2A`, plus 3 new rows this Work Package added for the `TD-67`/`TD-97` reconciliation services `WP 16.4B` registered in `TempestHost` Phase 6 without ever adding a register entry, found by the `v0.16.0` review board's Architecture perspective rather than by `WP 16.4B` itself. **The `WP 16.2A`-era inconsistency is now closed**: `docs/architecture/Platform Service Map.md` previously had no detail sections for the three `v0.13.0`/`v0.14.0` backfilled rows (`TD-126`) — `WP 16.4B-R1` added all three, plus sections and "At a Glance" rows for this Work Package's own three new rows, in the same pass. The four Engineering Foundation frameworks (`WP 7.1A`–`WP 7.1E`) received rows here and complete detail sections in `Platform Service Map.md` by `WP 9.8B`, closing the gap `WP 7.3A` first disclosed; Requirements Engine's own row remains current (`WP 7.3A`). |
...
| Bearing Library | Implemented | `A4` (Bearing Library, P01 Engineering Reference Data; design and implementation, no separate architecture-phase row here — see `docs/architecture/A4 Bearing Library.md`) — `IBearingCatalog`/`BearingCatalog`, the authoritative bearing reference-data catalogue: a typed index over `IEngineeringDocumentStore` (`Kind = "BearingReference"`) with its own `bearingId` and manufacturer-part-number `IPersistenceStore` indexes, a provenance-gated Draft→Checked→Validated→Released lifecycle, released-record immutability with supersession, deterministic query, and structured comparison. Supplies reference evidence only — no selection logic, no calculation methodology, no commercial data | ADR-0053, ADR-0055, ADR-0072, ADR-0073, ADR-0074, ADR-0124 |
| Bearing Validation | Implemented | `A4` (design and implementation, no separate architecture-phase row here) — `IBearingValidationService`/`BearingValidationService`, the bearing data-quality rules (`TEMPEST-BRG-001`…`022`) and the catalogue-wide `BearingDataQualityReport`. Read-only: reports, never repairs. Takes `IMaterialCatalog` as an **optional** collaborator, so a bearing can be recorded and checked before the material it names has been registered | ADR-0124 |
| Reference Data (shared layer) | Implemented | `Group A` (P01 Engineering Reference Data; see `docs/architecture/Group A Engineering Reference Data.md`) — `Tempest.Core.ReferenceData`, the provenance, Draft→Checked→Validated→Released→Superseded lifecycle, document-store-backed catalogue with typed and secondary indexes, sourced value/range types, comparison, validation-service shape and exception family every reference library shares. Not itself DI-registered: `IReferenceDataCatalog<T>` and `IReferenceValidationService<T>` are implemented per library, and each library's own concrete type is what the container holds | ADR-0053, ADR-0055, ADR-0072, ADR-0073, ADR-0074, ADR-0125, ADR-0126 |
| Standards Library | Implemented | `A2` (design and implementation; see `docs/architecture/A2 Standards Library.md`) — `IStandardCatalog`/`StandardCatalog`, the authoritative register of engineering standards (`Kind = "StandardReference"`). Records bibliographic identity, publisher status and cross-body equivalence; holds **no standard content**, and its `IStandardResolver` seam is how every other library confirms its own citations resolve | ADR-0126 |
| Standards Validation | Implemented | `A2` — `IStandardValidationService`/`StandardValidationService`, the standards register rules (`TEMPEST-STD-001`…`014`), including the guard against a scope summary long enough to be reproduced standard text | ADR-0126 |
| Materials Validation | Implemented | `A1` (uplift; see `docs/architecture/A1 Materials Database.md`) — `IMaterialValidationService`/`MaterialValidationService`, the materials rules (`TEMPEST-MAT-001`…`013`): property-name dimension checking, values physics forbids, and family-aware applicability | ADR-0055, ADR-0126 |
| Fastener Library | Implemented | `A3` (design and implementation; see `docs/architecture/A3 Fastener Library.md`) — `IFastenerCatalog`/`FastenerCatalog` (`Kind = "FastenerReference"`), covering bolts, screws, set screws, studs, nuts, washers, inserts, retaining rings, rivets and pins under one taxonomy. Records published torque figures; computes none | ADR-0126 |
| Fastener Validation | Implemented | `A3` — `IFastenerValidationService`/`FastenerValidationService`, the fastener rules (`TEMPEST-FST-001`…`020`) | ADR-0126 |
| Mechanical Components Library | Implemented | `A5` (design and implementation; see `docs/architecture/A5 Mechanical Components Library.md`) — `IComponentCatalog`/`ComponentCatalog` (`Kind = "ComponentReference"`), springs, gears, drive elements and standard machine components under one taxonomy, with three typed detail records gated by family traits | ADR-0125, ADR-0126 |
| Mechanical Components Validation | Implemented | `A5` — `IComponentValidationService`/`ComponentValidationService`, the component rules (`TEMPEST-CMP-001`…`024`), including which typed detail a family may carry | ADR-0126 |
| Engineering Constants Library | Implemented | `A6` (design and implementation; see `docs/architecture/A6 Engineering Constants.md`) — `IConstantCatalog`/`ConstantCatalog` (`Kind = "EngineeringConstant"`), dimensioned constant values with uncertainty and applicability, plus `IReleasedConstantSource`: the released-only seam a future calculation consumes a constant through | ADR-0126 |
| Engineering Constants Validation | Implemented | `A6` — `IConstantValidationService`/`ConstantValidationService`, the constants rules (`TEMPEST-CON-001`…`014`) | ADR-0126 |
| Manufacturing Process Library | Implemented | `A7` (design and implementation; see `docs/architecture/A7 Manufacturing Process Library.md`) — `IProcessCatalog`/`ProcessCatalog` (`Kind = "ManufacturingProcessReference"`), process families with capability bands, material compatibility, production scale and constraints. No process planning, no selection, no supplier capability | ADR-0126 |
| Manufacturing Process Validation | Implemented | `A7` — `IProcessValidationService`/`ProcessValidationService`, the process rules (`TEMPEST-MFG-001`…`017`) | ADR-0126 |
...
| Project Engine | Not implemented as a platform service. The bootstrap-era code this row used to cite (`Tempest.Core.Projects`, `ProjectService`, `JsonProjectRepository`) was **deleted by `WP-C`** (`TD-110`) as demonstrably dead; the live project capability is `Tempest.App.Projects` (`ProjectContext`, `ProjectDirectory`, `ProjectGovernanceService`), which is an App-layer capability rather than a Core platform service | Planned, no Work Package assigned | None |
...
| User Command Macros | Implemented | WP 10.6A (design and implementation, no separate architecture-phase row — a "foundation" capability per its own controlling instruction) | ADR-0099 |
| External Controller / Input Binding | Implemented | WP 10.6A (design and implementation, no separate architecture-phase row) — one real Keyboard provider, one test-only stub controller; no real vendor SDK integration, by explicit Out-of-Scope | ADR-0100 |
| Plugin Trust & Capability Enforcement | Implemented | `v0.13.0` (design and implementation, no separate architecture-phase row here) — trust store, tier assignment, signature verification, dependency graph validation, denied-type/component-principal registries (`IPluginTrustStore`, `IPluginRegistry`, `IPluginDeniedTypeRegistry`, `IPluginComponentPrincipalRegistry`) — a genuine, new Platform Service beyond the original `v0.4.0` Plugin Manifest row (trust/authorization, not manifest parsing); backfilled `WP 16.2A` | ADR-0107, ADR-0108, ADR-0109, ADR-0110, ADR-0111, ADR-0112 |
| Engineering Object Durability & Rehydration | Implemented | `v0.14.0` (design and implementation, no separate architecture-phase row here) — `IEngineeringObjectStateStore`/`EngineeringObjectState` (`TD-85`), `IEngineeringObjectRehydrator`/`IEngineeringObjectRehydratorRegistry`/`EngineeringObjectRehydrationService`, `IRehydratable<TSelf>` — what makes an engineering object survive a process restart; backfilled `WP 16.2A` | ADR-0113, ADR-0116 |
| Attachment Content Store | Implemented | `v0.14.0` (design and implementation, no separate architecture-phase row here) — `IAttachmentContentStore`/`IBinaryPersistenceStore`, the durable store of attachment *bytes*, distinct from `IPersistenceStore`'s text-valued store; backfilled `WP 16.2A` | ADR-0113, ADR-0114 |
| Requirements Reconciliation | Implemented | `WP 16.4B` (design and implementation, no separate architecture-phase row here) — `IRequirementsReconciliationService`/`RequirementsReconciliationService`, the reconcile/repair path `TD-67` named as absent for an orphaned Requirement/Collection/Group document; explicit `DetectAsync`/`SweepAsync` only, invoked by nothing — no startup hook, no command, no user-facing surface; added by the `v0.16.0` review board's Architecture remediation (`WP 16.4B-R1`), not by `WP 16.4B` itself | None dedicated (reuses ADR-0053, ADR-0058) |
| Material Catalog Reconciliation | Implemented | `WP 16.4B` (design and implementation, no separate architecture-phase row here) — `IMaterialCatalogReconciliationService`/`MaterialCatalogReconciliationService`, the sibling sweep for `MaterialCatalog`'s own `materialId` index (`TD-67`, `ADR-0055` Decision 3); explicit `DetectAsync`/`SweepAsync` only, invoked by nothing; added by `WP 16.4B-R1` | ADR-0055 |
| Attachment Content Reconciliation | Implemented | `WP 16.4B` (design and implementation, no separate architecture-phase row here) — `IAttachmentContentReconciliationService`/`AttachmentContentReconciliationService`, the sweep closing `ADR-0114` Decision 4's own disclosed crash window for orphaned attachment content (`TD-97`); explicit `DetectAsync`/`SweepAsync` only, invoked by nothing; added by `WP 16.4B-R1` | ADR-0114 |
| Attachment Write Intent | Implemented | `WP 16.4B-R2` (design and implementation, no separate architecture-phase row here) — `IAttachmentWriteIntentStore`/`AttachmentWriteIntentStore`, the durable marker recording that an attachment's content write is in flight. Marked before the content write and cleared only after the owning object's state write succeeds, so the Attachment Content Reconciliation sweep can skip content whose owning reference has not landed yet. Sampled by that sweep **between** its content-key read and its object-state read — the position, not the marker alone, is what makes the ordering airtight. A marker can only ever *prevent* a collection, never cause one: a crash leaving a stale marker costs unreclaimed bytes, which is the outcome `TD-97` already tolerated. Added here at the independent post-remediation review, which found it registered in `TempestHost.cs` Phase 6 with no row — the same §6 currency defect `WP 16.4B-R1` had just closed for its three sibling services, recurring one Work Package later because the two remediation streams ran on separate branches and deliberately did not touch each other's registers | ADR-0113, ADR-0114 |

**Total: 41 entries — 39 Implemented, 1 planned with no code (Project
Engine), 1 developer-convenience layer (Module SDK). Two new rows added
by `A4`** (Bearing Library, Bearing Validation) for the `P01`
Engineering Reference Data bearing capability; 39 → 41. **Corrected in
the same pass:** this line read "38 entries" while the **Coverage
Status** field above already read "39 of 39", because `WP 16.4B-R2`'s
own Attachment Write Intent row was added to the table and to Coverage
Status but not to this line — a one-row arithmetic drift, counted
directly against the table above rather than carried forward.
**Three rows added by `WP 16.4B-R1`** (Requirements Reconciliation, Material Catalog
Reconciliation, Attachment Content Reconciliation) for the `TD-67`/
`TD-97` sweep services `WP 16.4B` registered in `TempestHost` Phase 6
without ever adding a register row for any of the three — found by the
`v0.16.0` review board's Architecture perspective, not by `WP 16.4B`
itself, whose own Definition of Done should have covered this per
Engineering Governance §6; 35 → 38. **Three further new rows backfilled
by `WP 16.2A`** (Plugin Trust & Capability Enforcement,
Engineering Object Durability & Rehydration, Attachment Content Store)
for `v0.13.0`/`v0.14.0` Platform Services that shipped without a
register row — 32 → 35. `docs/architecture/Platform Service Map.md`'s
own equivalent detail sections for all six of the above are now
complete — `WP 16.4B-R1` added them for the three `WP 16.2A` backfilled
(closing `TD-126`) and for its own three new rows, in the same pass;
see this register's own **Last Reviewed** field below. Two new rows added
by `WP 10.6A` (User Command Macros, External Controller / Input
Binding) — both genuine, new, DI-registered Platform Services
(`IMacroManager`, `IInputBindingRegistry`), not folded into the
existing Command Framework row, since each carries its own real ADR
and its own distinct responsibility (macro persistence/registration;
input-source routing) beyond what `ICommandDispatcher`/`ICommandRegistry`
themselves provide. Four new rows added
