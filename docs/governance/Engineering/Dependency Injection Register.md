...
**Total: 30 individually-named registrations above (two of which —
...
a dynamic set of discovered types. Verified directly: 32 `Singleton`/
`AddInstance` call sites plus 2 `AddDiscovered*` call sites = 34 total
registration statements in `TempestHost.cs`'s own Phase 6 block
(`grep -n "services\.\(Singleton\|AddInstance\)" src/Tempest.Core/
Runtime/TempestHost.cs` returns 32; adding the 2 `AddDiscovered*` lines
gives 34), matching this table's own 30 named single/dual registrations
(30 rows, accounting for 32 raw `Singleton`/`AddInstance` calls) plus 2
discovered-type rows exactly. Four of these — `IEngineeringDocumentStore`,
`IMaterialCatalog`, `ICalculationEngine`, `IVerificationService` — were
added by the Engineering Foundation programme (`WP 7.1A`, `WP 7.1C`,
`WP 7.1D`, `WP 7.1E` respectively) and had never been recorded in this
register before `WP 7.1F`'s own backfill; `Tempest.Core.UnitsAndQuantities`
(`WP 7.1B`) registers nothing, by design (`FCR-0030`'s own "zero Platform
Service dependency and no DI registration of any kind").**
