...
**21 + 1 + 3 + 1 + 1 = 27 — matches the Entries table's own row count exactly, both independently re-derived (direct table row count; direct per-row Status-column classification), not reconciled by adjusting one to match the other.**
