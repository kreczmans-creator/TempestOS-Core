...
- [WP 16.4B — Durability & Loopback Hygiene](03%20Work%20Packages/WP16.4B-durability-and-loopback-hygiene.md) — closes `TD-62`/`TD-68`/`TD-69`/`TD-97` and partially `TD-67`; records the data-loss defect it shipped, and the correct refusal to "fix" a stale `TD-67` claim against an already-tested `WP 9.1A` design.
- [WP 16.4B-R1 — Migration Collision Guard and Platform Service Registration](03%20Work%20Packages/WP16.4B-R1-migration-collision-guard-and-platform-service-registration.md) — closes a `StateMigrationRegistry` collision hazard and `TD-126`; ships with no `allowReplace` escape hatch, unlike its DI sibling, and its own "No ADR" judgement was later overturned as `ADR-0123`.
- [WP 16.1A-R1 — Release Path Enforcement](03%20Work%20Packages/WP16.1A-R1-release-path-enforcement.md) — the release path was weaker than the merge path `WP 16.1A` had just hardened; catches a CI run completing `cancelled`, not `failure`, which a naive gate would have waved through.
- [WP 16.2C — v0.16.0's Own Academy Retrospectives](03%20Work%20Packages/WP16.2C-academy-retrospective-completion.md) — the Work Package that closed the retrospective gap, and left six Work Packages uncovered on its first pass, including itself.
- [WP 16.4B-R2 — Attachment Sweep Write-Intent Marker](03%20Work%20Packages/WP16.4B-R2-attachment-sweep-write-intent-marker.md) — closes the data-loss race the review board found; the first attempt was rejected for sampling the marker in the wrong position, which only narrowed it.
- [WP 16.5A-R1 — Accessibility Remediation](03%20Work%20Packages/WP16.5A-R1-accessibility-remediation.md) — six findings closed, including a focus ring invisible on the Primary button and a test that structurally could not catch it; records that Avalonia resolves same-selector styles by addition order, not specificity.
