...
- [WP 14.2A — Companion Android & iOS Platform Heads](03%20Work%20Packages/WP14.2A-companion-android-and-ios-platform-heads.md) — the official Avalonia library/heads split (`Tempest.Companion` becomes the shared app library; Desktop/Android/iOS heads over it), heads outside the gating solution with a dispatch-only `mobile-heads.yml` build (`ADR-0116`), brand-pack launcher icons, and validation graded honestly to the environment's edge (a reference-assembly compile that caught a real AppCompat launch crash). `TD-57` → Partially resolved.
- [WP 14.2.1 — CI Pipeline Remediation](03%20Work%20Packages/WP14.2.1-ci-pipeline-remediation.md) — the required `CI Gate` had been red on this branch *and* on `main` at commits whose build and tests both passed: an exhausted Actions artifact-storage quota failed the upload steps, and the gate keyed off a job conclusion that folded them in. `ADR-0117` separates verification from diagnostics, removes the ~50 MB-per-push cause, and makes every failed test name itself in the Job Summary. Added `TD-59` (a real intermittent Release-configuration test, disclosed not retried) and `TD-60` (Node 20 action deprecation).
