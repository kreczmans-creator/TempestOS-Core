...
(() => {
  ...
  class DocPage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Self-healing: streaming hosts (DC runtime) can commit children in ways
      // that race the initial measure. For the first 10s, whenever the presence
      // of explicit .page sections disagrees with the paginated class, force a
      // synchronous re-measure.
      if (this._healTimer) clearInterval(this._healTimer);
      let healTries = 0;
      this._healTimer = setInterval(() => {
        healTries++;
        if (this._sheet) {
          const want = !!this.querySelector(':scope > .page');
          const has = this._sheet.classList.contains('paginated');
          if (want !== has) this._measure();
        }
        if (healTries >= 40) { clearInterval(this._healTimer); this._healTimer = null; }
      }, 250);
    }
