...
## Addendum (`WP 21.3B`) — expenses and purchase orders are engineering objects on the project too; still no ledger

`D-028`'s "not ERP, not PLM" holds through the two commercial edges a
consultancy hits earliest, and both repeat §1–§5's shape rather than
invent a new one. **`ProjectExpense : EngineeringObjectBase, IRehydratable<ProjectExpense>`**
(`src/Tempest.Core/Expenses/`), parented to the project like
`TimesheetEntry`: a date, a description, a closed `ExpenseCategory`
(Travel/Subsistence/Materials/Subcontract/Other — a vocabulary, not a
free-text account code), a net amount, a VAT amount, a receipt through
the existing `IHasAttachments` seam rather than a new one, `Billable`,
and `InvoicedBy` (`TimesheetEntry`'s own once-only rule, unchanged).
`ExpenseService.RecordAsync` refuses a currency that does not match the
project's own and a negative amount; nothing here computes a balance or
posts to an account — a billable expense's only downstream effect is
becoming one more line `InvoicingService.RaiseAsync` sweeps in beside
completions and timesheet entries, exactly as §4's `DeliverableCompletion`
already does.

**`PurchaseOrder : EngineeringObjectBase, IRehydratable<PurchaseOrder>`**
(`src/Tempest.Core/PurchaseOrders/`), parented to the project, reference
`PO-<yyyy>-<nnn>` generated by the identical scan-the-store discipline
`QuotationService.NextReferenceAsync` already uses for `Q-`/`CO-` — no
sequence table, no counter row, because there is still no ledger to keep
one in. A supplier is an Organisation-catalogue id, tagged and never
validated, exactly `Project.ClientOrganisationId`'s own rule (§1). Lines
carry description/quantity/unit price/net/VAT with net and gross
computed, never stored twice. Status is `Draft → Issued → Received →
Closed | Cancelled`, a permitted-transition table the same shape as
`Evidence`'s own status machine, not a workflow engine: there is no
approval chain, no budget check against `Project.Budget`, and no
three-way match against a receipt — those are the procurement and
accounting-system features `D-028` names as out of scope, and a received
order's lines becoming expenses (`RecordLinesAsExpensesAsync`, one act,
one audit row, category `Materials`, `Billable: true`) is as far as this
Work Package carries them. `PurchaseOrder` records what was ordered and
that it arrived; what it cost the project is the `ProjectExpense` rows
it produces, read the same way every other expense is.

**VAT is a rate on a line, not a tax engine.** `VatRate` (`src/Tempest.Core/BusinessGovernance/`)
is a five-value closed vocabulary — Standard 20%, Reduced 5%, Zero,
Exempt, Out of scope, the last one its own default so every
`QuotationLine`/`InvoiceRequestLine` recorded before this Work Package
reads unchanged — carried on the line, never computed from a
jurisdiction table or a customer's own tax status; `VatAmount` is
`NetAmount × Percentage()`, arithmetic, not policy. The connector seam
(`ADR-0151`) maps each of the five to the one Xero tax type that means
it (`OUTPUT2`/`RROUTPUT`/`ZERORATEDOUTPUT`/`EXEMPTOUTPUT`/`NONE`) and
refuses, as a `Rejected` result rather than a guess, a line the map does
not cover — no VAT return, no reclaim, no multi-jurisdiction rate
resolution is built or implied.

**The second principal is a directory lookup, not an identity system.**
Settings → Principal's `Switch person…` reads `IPeopleDirectory` —
`WP 20.10F`'s own People library where it has merged, a small in-memory
stand-in registered the same way when it has not — confirms by name with
no password, and calls the existing `ICurrentPrincipalAccessor.SetCurrent`.
Nothing new authenticates; `EvidenceService.RecordCheckAsync`'s existing
checker-is-recorder refusal (`ADR-0148`) is what this switch exists to
satisfy, and its message now names the fix directly rather than leaving
a user to infer it.

No new store, no new transaction shape, no field that does not trace to
a calc sheet, a drawing, or the invoice seam this Work Package's own
brief named. Unlike §5's two Kinds, these two earn their own small
discipline workspace each — `ExpenseWorkspaceRegistration` (project →
category → expense) and `PurchaseOrderWorkspaceRegistration` (project →
status → order) — because both are things a consultancy files and
browses on their own account, not only through a project's existing
tabs; each still registers a production rehydrator the same way, the
twenty-fourth and twenty-fifth Kinds after Evidence's eighteenth.
