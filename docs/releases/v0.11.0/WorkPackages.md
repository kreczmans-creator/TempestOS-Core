...
(Branch Protection & Engineering Workflow Hardening), and `WP 11.2A`
(Governance Health-Check Automation) are this release's own first five
Work Packages, each following this project's own standing discipline
(`FOUNDATION.md` §1: architecture and planning precede implementation
for anything non-trivial) — `WP 11.0A`/`WP 11.0B` review-and-planning
only; `WP 11.1A`/`WP 11.1B`/`WP 11.2A` implementation, scoped entirely to
CI/release-engineering/governance-tooling infrastructure, with no
production code, ADR, or architecture change in any of the three. This
...
(`WP 11.1B`), and its first automated governance health check
(`WP 11.2A`) all now exist.
...
`A-2`) has now been predicted for two Work Package slots without being
commissioned under either — it remains genuinely not started, carried
forward unscoped to a specific future Work Package number — see
`PROJECT_STATUS.md`, Next Planned Work Package.
...
| — | Desktop & Console Presentation Strategy Decision (`WP11.0A` finding `A-2`) — predicted for both `WP 11.2A` (original roadmap) and implicitly the next slot (this document's own prior revision); commissioned under neither. Genuinely not started; unscoped to a specific future Work Package number. Still the single highest-leverage undecided item in the roadmap. | Architecture / Decision | Not started |
