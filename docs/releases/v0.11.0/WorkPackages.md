...
**Released and published.** All nine `v0.11.0` Work Packages
(`WP 11.0A` through `WP 11.4A`) are now complete. `WP 11.9.0`'s own
Engineering Release Report recommended **Accept with Observations**
(`docs/releases/v0.11.0/WP11.9.0 Engineering Release Report.md`);
Product Approval was then sought and given, and `v0.11.0` was tagged,
pushed, and published as a real GitHub Release with two correctly-
separated assets (`Tempest.Desktop`, `Tempest.App`/Internal Engineering
Harness) — the release's own `Release Publish` workflow independently
rebuilt and retested it on real infrastructure before publishing. That
same push gave this release its first-ever real CI execution, which
found three genuine, previously-invisible defects (none a regression in
product code) — `WP 11.4A` fixed and independently re-verified all
three on real infrastructure (`docs/releases/v0.11.0/WP11.4A Release
Engineering Corrections.md`; full diagnosis in `Release Publication
Report.md`). `Build & Test` and `CI Gate` now genuinely pass on real
GitHub-hosted infrastructure. Branch protection remains not yet
configured (`TD-45`) — recommended before merging `main`, per both
documents' own recommendation.
