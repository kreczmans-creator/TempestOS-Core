...
| `ADR-0154` | Time, deliverable completion and rate resolution are engineering objects on the project; billing and cost rates freeze at entry; the five KPI equations and their provenance are defined here; TempestOS holds no ledger | `WP 19.0A`, `WP 19.1B` |
| `ADR-0155` | Invoicing is an outbound connector to the accounting system; the request id is the idempotency key and the invoice reference; TempestOS creates draft invoices and reads status; it never marks anything paid | `WP 19.1A` |
