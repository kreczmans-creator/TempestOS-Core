...
| `WP 18.0B` Reference libraries with citation (ADR-0149) | `SourceCitation` on every reference record, carried forward on revise; supersession invariant proved by property test over all five libraries; all 41 seeded records cited | 2026-09-09 |
...
| `WP 18.1A` Async read surface | *(filled at merge)* | |
| `WP 18.1B` Findability | *(filled at merge)* | |
| `WP 18.2A` Evidence workspace | *(filled at merge)* | |
| `WP 18.2B` Independent check and issue sheet | *(filled at merge)* | |
