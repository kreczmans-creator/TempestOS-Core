...
| `WP 18.9.1` First Windows run | The Libraries tab never loaded: `EvidenceWorkspaceView.RefreshAsync` reloaded the evidence list on entering the area but never asked the Libraries view to load, and every test had refreshed it by hand; the area now loads both, a regression test reaches the tab the way the application does, and Add Material refuses a blank name or designation in plain words instead of a raw argument message | 2026-09-10 |
