...
**10 findings fixed in this package**, across 1 High, 5 Medium, and 4 Low severity issues (`OSA-06` bundles one Medium and one Low sub-issue under a single row; `OSA-11`, `THIRD-PARTY-NOTICES.md`, is a documentation-completeness item rather than a severity-rated vulnerability — see the table for the row-by-row breakdown). **4 findings recorded as accepted, deferred risk** (all Low/Informational, all architectural and requiring a design decision this package's own brief does not authorise it to make unilaterally — mirroring this platform's own established precedent for exactly this situation, `Platform Security Review v0.5.0.md`'s SEC-01/NAV-1). **1 finding sized as a feature and recommended as a new Work Package** (audit-log coverage for three services, Medium). **No Critical finding.** Two attack surfaces the brief named do not yet exist in this codebase (the SVG viewer, `WP 21.4A`; backup/restore and the installer, `WP 21.5A`) — both are reviewed for the requirement they must satisfy when built, not fixed, since there is no code yet to fix.

| Severity | Fixed here | Deferred (accepted risk) | Recommended as new WP | No finding (reviewed) |
|---|---|---|---|---|
| Critical | 0 | 0 | 0 | — |
| High | 1 | 0 | 0 | — |
| Medium | 5 | 0 | 1 | — |
...
## Findings recorded as accepted, deferred risk (architectural — not fixed)

These four all share one root cause and one precedent for deferring them: `ICurrentPrincipalAccessor`'s concrete implementation, `CurrentPrincipalAccessor`, is registered in the DI container under its own concrete type (`TempestHost.cs:407-409`, `services.AddInstance(currentPrincipalAccessor)`) with `SetCurrent` public on that concrete type — reachable by any in-process component that resolves it, not only `WorkspaceHost` (its own documented "sole caller" claim is already not enforced in code, and is already not true in practice: `src/Samples/Tempest.Samples/SamplePrincipalFactory.cs:29-38` calls `accessor.SetCurrent(...)` with an arbitrary caller-supplied identity string and no credential check). This is architecturally identical to `Platform Security Review v0.5.0.md`'s own SEC-01 finding (no isolation boundary between a loaded plugin and a first-party module) and is deferred for the identical reason: **there is no live untrusted actor to exploit it today.** Every module in this build is first-party and trusted (single trust domain, `Threat Model.md`'s own "Trust Boundaries" section), and — confirmed directly by this audit, above — the one mechanism that would introduce an untrusted in-process actor (plugin assembly loading) is unreachable (`src/Frozen/`). Building an access-control model for a threat actor that cannot yet run any code in this process would be exactly the "speculative improvement" this platform's own prior audit already declined to build ahead of a live need (`Platform Security Review v0.5.0.md`, SEC-02's own disposition). Each becomes a live concern, in this identical order, the day `src/Frozen/Tempest.Core.Plugins` (or any third-party code-loading path) ships — the same trigger `Platform Security Review v0.5.0.md` already named for SEC-01/TD-09.

### OSA-12 — `CurrentPrincipalAccessor.SetCurrent` is reachable by any DI-resolving component (Low / Informational) — **NOT FIXED**

- **File:line.** `src/Tempest.Core/Identity/CurrentPrincipalAccessor.cs:50-72`; `src/Tempest.Core/Runtime/TempestHost.cs:407-409`; demonstrated already in use outside its documented sole caller at `src/Samples/Tempest.Samples/SamplePrincipalFactory.cs:29-38`.
- **Exploit (prospective).** A future untrusted in-process component (a third-party plugin, once `src/Frozen/`'s loader — or any replacement for it — ships) could call `SetCurrent` with an arbitrary identity, falsely attributing every subsequent write in the process to that principal, or `SetCurrent(null)` to force the "unknown actor" fallback (OSA-14) and hide who did something.
- **Recommendation.** When plugin loading (or any other untrusted in-process code path) is designed, give it its own review alongside — narrow `SetCurrent`'s reachability (an internal seam, or a capability the DI container itself gates) as part of that same design, not as a standalone patch now with nothing yet to protect.

### OSA-13 — The audit collection has no access control at the generic store layer (Low / Informational) — **NOT FIXED**

- **File:line.** `src/Tempest.Core/Audit/AuditRecorder.cs`; `src/Tempest.Core/Runtime/TempestHost.cs:435-437` (`IPersistenceStore`/`IQueryablePersistenceStore` registered as an ordinary, broadly-injectable DI singleton).
- **Exploit (prospective).** Any code holding an `IPersistenceStore` reference (several first-party services legitimately do, for their own indexes) can call `DeleteAsync("Audit", key)` or `WriteAsync("Audit", key, forgedJson)` — nothing distinguishes the `Audit` collection from any other at the generic store layer. No ordinary command/service surface exposes this today (`IAuditRecorder`/`IAuditQuery` expose no deletion at all); the test suite itself demonstrates the underlying mechanic only to simulate corruption for its own tests (`AuditQueryCorruptionTests.cs:24-25`).
- **Recommendation.** Same trigger and same disposition as OSA-12 — a per-collection access-control layer on `IPersistenceStore` is a real architectural addition, correctly out of this package's own scope to design unilaterally, and has no live actor to protect against today.

### OSA-14 — The "unknown actor" audit fallback is forceable via the same root cause (Low / Informational) — **NOT FIXED**

- **File:line.** `src/Tempest.Core/Audit/AuditRecorder.cs:59,100`; `src/Tempest.Core/EngineeringData/EngineeringDocumentStore.cs:62`.
- **Finding.** No write-path parameter lets a caller pass an arbitrary actor id or force the fallback directly; the fallback triggers purely on `CurrentPrincipalAccessor.Current == null`. Because `Current` is the same ambient, mutable singleton OSA-12 names, a component that can reach `SetCurrent` could force it to `null` immediately before an otherwise-attributable action. Same root cause, same disposition, not duplicated as a separate design item.

### OSA-15 (recommend new Work Package) — `RequirementsService`, `VerificationService`, and `ReferenceDataCatalog` write durable engineering data with no audit row (**Medium**) — **NOT FIXED — recommend `WP 21.6`**

- **Surface.** The audit log (brief item 9).
- **Severity reason.** Medium: an accountability/non-repudiation gap against this platform's own stated invariant ("every write is one transaction with an audit row") — not a path to unauthorised access, data exposure, or code execution, and not something an external attacker gains new capability from; a real gap for anyone later asking "who changed this requirement, and when."
- **Finding.** Every mutation through `EngineeringObjectBase`/`EngineeringObjectFactory` (the canonical engineering-object path — CAD parts, drawings, and the rest) unconditionally writes an audit row inside the same transaction (`EngineeringObjectBase.cs:164-201`, `WriteAuditAsync` at `:295-297`, unconditional, no opt-out). Three services commit real, durable engineering data through their **own**, separate `IQueryablePersistenceStore.ExecuteInTransactionAsync` calls, bypassing `EngineeringDomainContext.ExecuteWriteAsync` (and so the audit machinery) entirely, with **zero calls to `AuditTransactionWriter`/`IAuditRecorder` anywhere in any of the three files** (confirmed by direct grep):
  - `src/Tempest.Core/Requirements/RequirementsService.cs` — all 14 mutators: `CreateAsync:190`, `ReviseAsync:245`, `SetStatusAsync:271`, `SetOwnerAsync:291`, `SetPriorityAsync:315`, `DeleteAsync:339`, `MoveToGroupAsync:363`, `LinkAsync:397`, `CreateCollectionAsync:443`, `DeleteCollectionAsync:492`, `AddToCollectionAsync:521`, `CreateGroupAsync:540`, `MoveGroupAsync:611`, `DeleteGroupAsync:652`.
  - `src/Tempest.Core/Verification/VerificationService.cs` — `RecordAsync:162-213`.
  - `src/Tempest.Core/ReferenceData/ReferenceDataCatalog.cs` — `RegisterAsync:192`, and the write paths at `:344`, `:450` (note: the separate *review/approval* workflow, `ReferenceReviewService.cs:187,259`, **does** call `IAuditRecorder.RecordAsync` — only raw registration is unaudited).
- **Why this is recommended as a new Work Package rather than fixed here.** Auditing all three services correctly is not a small, isolated patch: it is ~20 call sites across three services with three different write shapes, each needing a considered action-name taxonomy and a detail payload that says something genuinely useful (not merely "something changed"), plus its own test coverage proving each audit row's content — squarely "a feature of its own," per the brief's own carve-out. A rushed version risks either under-detailed rows that satisfy the letter of the invariant but not its purpose, or an inconsistent shape against `EngineeringAuditActions`' own existing naming convention.
- **Recommendation.** A new `WP 21.6` (or the next available number): extend `EngineeringAuditActions`-style named constants and wire `AuditTransactionWriter.WriteAsync` into each of the ~20 call sites above, inside their existing `ExecuteInTransactionAsync` blocks (the transaction handle is already in scope at every site) — mechanically straightforward once the action-name/detail design is settled, which is the part this package should not improvise.
