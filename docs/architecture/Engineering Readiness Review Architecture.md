...
|---|---|---|
| **NOT READY** | §4 row 1: any category Not Ready | **Used.** *(Corrected `WP 16.4B-R6` round 2, 2026-09-06: this cell read "Not yet used; the case this taxonomy exists to make unambiguous", which was already false when written — `WP 12.9.0` and `WP 12.9.2` both derived `NOT READY` on Verification readiness, and `WP 12.9.4` records its own verdict moving from `NOT READY` to `ACCEPT WITH OBSERVATIONS` in place. It is now false a second way: `v0.16.0`'s Engineering Release Report recommends `NOT READY` on one open Release Blocking finding, `TD-143`. *(`TD-143` corrected from `TD-140` by the round-2 board, 2026-09-06: `TD-140` is Resolved for the supersession-refusal route, and the blocker moved to `TD-143`, which carries the same demonstrated harm by the failing-durable-write route. Citing the Resolved row here pointed a reader towards concluding the recommendation had been discharged.)* *(**`TD-147` corrected from `TD-143` by the `WP 16.4B-R7` governance closure — seam reconciliation, 2026-09-07, derived from the Technical Debt Register and the tree at `d6af7ec`. This trail is EXTENDED, not overwritten: both earlier corrections above stand as written.** `TD-143` is now **Resolved for the seven mutators on `EngineeringObjectBase` and for nothing else**, and its own row states in its first sentence that it does **not** close the harm class it was raised for. The blocker moved a second time, to **`TD-147`** — an object creation whose initial durable state write fails still registers the object, and its next successful write makes a creation the caller was told had failed into a real, durable one — which carries the identical demonstrated harm on an eighth path and was opened Release Blocking in the same pass that scoped `TD-143`. **The recommendation is unchanged: `NOT READY`, on one open Release Blocking finding. The blocker MOVED; it did not disappear, and this precedent cell is still `Used`.** The hazard the previous correction names is exactly the hazard here, one round later and in the same cell: `TD-143` now reads `Resolved`, so a reader following this cell to that row could conclude the recommendation had been discharged. It has not been. That recurrence is why the identity is corrected in the same pass that changed the row, and why the two records were moved together rather than one at a time.)* Corrected here only because leaving it would contradict a live release document; the pre-existing `v0.12.0` inaccuracy is recorded, not investigated further, as it is outside that Work Package's scope.)* |
