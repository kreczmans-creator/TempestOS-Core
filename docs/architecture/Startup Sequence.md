...
**Update, WP 17.2A (ADR-0146):** Plugin Loading, and the licence
validation phase this diagram never showed to begin with (`ADR-0050`,
frozen by `ADR-0146`), are removed below. Plugin Discovery stays exactly
as diagrammed — it discovers, validates and records a candidate; nothing
loads, signs, verifies, scopes or enforces it further.
...
sequenceDiagram
    ...
    Note over Host,PluginDiscovery: Manifest discovery only - WP 4.2 (Tempest.Core.Plugins), per ADR-0026.<br/>Loading, signing and trust enforcement are frozen (ADR-0146, WP 17.2A) -<br/>see src/Frozen/README.md.
    ...
    PluginDiscovery-->>Host: IReadOnlyList~PluginManifest~ (validated, discovered)
    Note over Host: Plugin Discovery - recorded as PluginRegistryState.Discovered;<br/>goes no further
    end
...
- Plugin-scoped failures (a malformed manifest, an incompatible version)
  never appear as a `--x` arrow to the Host in this diagram — per
  ADR-0025, they are isolated to that one plugin, logged at their
  assigned severity, and excluded, exactly how an individual module's
  failure never appears as a thrown exception during Module Initialisation
  either. Only a genuine defect in Plugin Discovery's own orchestration is
  Host-fatal.
...
| ~~Plugin Loading~~ | — | Frozen (ADR-0146, WP 17.2A) — no longer a phase; nothing loads what Discovery finds |
