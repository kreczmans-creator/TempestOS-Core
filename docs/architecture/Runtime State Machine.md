...
**Update, WP 4.5 (design phase):** ADR-0029/ADR-0030's two new lifecycle
*phases* (Hosted Services Started, Hosted Services Stopped — see
`Host Lifecycle.md`) are designed, occurring entirely within the existing
`Starting`/`Stopping` states respectively. **No new state and no new
transition are introduced.** A critical hosted service's failure uses
exactly the `Starting → Faulted`/`Stopping → Faulted` transitions this
document already defines for a platform-service failure and a genuine
shutdown-time Host-level defect, respectively — not a new transition of
its own. This remains a design-only update; nothing below changes until
`WP 4.5` implements it.
...
| `Starting` | `Running` | Every platform-service phase, Module Initialisation, and Hosted Services Started (ADR-0029/0030, designed — WP 4.5) completed (regardless of individual module/hosted-service outcomes). |
| `Starting` | `Faulted` | A platform-service failure (Configuration/Logging/DI/Discovery/Registration) — ADR-0013 — or a critical hosted service's `StartAsync` failure (ADR-0021/0029, designed — WP 4.5, not yet implemented). |
...
| `Stopping` | `Stopped` | Hosted Services Stopped, Module Disposal, and Service Disposal all completed (individual module Stop/Dispose failures already isolated by WP 2.3, and individual hosted-service Stop failures per ADR-0021/0029 once implemented, do not prevent this). |
| `Stopping` | `Faulted` | A genuine Host-level defect in shutdown orchestration itself, or a critical hosted service's `StopAsync` failure (ADR-0021/0029, designed — WP 4.5, not yet implemented) — not an ordinary module or isolated hosted-service failure. |
