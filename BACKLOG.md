...
| `TD-175` | A Part has no BOM input at all: the bill of materials is authored on the Assembly (its lines: child, quantity, find number, item number, reference designator), and a Part shows only a read-only **Where used** readout derived from the assemblies that include it. Today the line lives on the child (`IHasBomLine`) and the editor offers it as "Bill of Materials" on the Part. A meaningful Where-used also needs a Part to be usable in more than one assembly, which the single-parent tree (ADR-0081, `IHasParent`) does not allow: the part definition and its occurrences must be distinguished (Product Owner, 2026-09-09) | `WP 18.1C` (model) and `WP 18.2A` (page) |
