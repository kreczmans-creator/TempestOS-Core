...
| `IReferencePinResolver` | `Tempest.Core.ReferenceData` | DI-public | Resolves a `ReferencePin` into one library, so a record's pinned sources can be checked without the checking code depending on every library it might cite; moved here from `CommercialIntelligence.Estimating` at `Group E` (`ADR-0137`) |
...
| `ITemplateCatalog` | `Tempest.Core.EngineeringAssets.Templates` | DI-public | The engineering template library, returning released templates most specific first and pinning a template at the revision actually read (`E1`, `ADR-0136`) |
| `ITemplateValidationService` | `Tempest.Core.EngineeringAssets.Templates` | DI-public | Governance of templates, including a mandatory section that requires nothing and a choice field offering no choices (`E1`, `ADR-0136`) |
| `ICalculationPackCatalog` | `Tempest.Core.EngineeringAssets.CalculationPacks` | DI-public | The calculation-pack library, searchable by method and by the record revisions a pack rests on (`E2`, `ADR-0137`) |
| `ICalculationPackValidationService` | `Tempest.Core.EngineeringAssets.CalculationPacks` | DI-public | Governance of calculation packs, including an unsourced input and an unnamed solver version (`E2`, `ADR-0137`) |
| `IVerificationArtefactCatalog` | `Tempest.Core.EngineeringAssets.Verification` | DI-public | Verification evidence against requirements, with the outstanding queue ordered failures first (`E3`, `ADR-0138`) |
| `IVerificationArtefactValidationService` | `Tempest.Core.EngineeringAssets.Verification` | DI-public | Governance of verification artefacts, including the error raised where a pass has no locatable evidence behind it (`E3`, `ADR-0138`) |
| `IVerificationTraceService` | `Tempest.Core.EngineeringAssets.Verification` | DI-public | Answers whether a requirement is verified and how anybody knows, reporting concerns and changing nothing (`E3`, `ADR-0138`) |
| `IDesignReviewCatalog` | `Tempest.Core.EngineeringAssets.DesignReviews` | DI-public | Design review packs, and every unclosed action across every review most overdue first (`E4`, `ADR-0138`) |
| `IDesignReviewValidationService` | `Tempest.Core.EngineeringAssets.DesignReviews` | DI-public | Governance of design reviews, including the error raised where a review proceeds over an unanswered critical observation (`E4`, `ADR-0138`) |
| `ITechnicalDocumentCatalog` | `Tempest.Core.EngineeringAssets.TechnicalDocumentation` | DI-public | Governed technical documents, with the set actually in force on a stated date (`E5`, `ADR-0137`) |
| `ITechnicalDocumentValidationService` | `Tempest.Core.EngineeringAssets.TechnicalDocumentation` | DI-public | Governance of technical documentation, including an issued document with no issue revision and a predecessor still in force (`E5`, `ADR-0137`) |
| `IPromptCatalog` | `Tempest.Core.Knowledge.Prompts` | DI-public | The prompt library — a library, not a runtime: nothing here executes a prompt or binds a model (`F1`, `ADR-0140`) |
| `IPromptValidationService` | `Tempest.Core.Knowledge.Prompts` | DI-public | Governance of prompts, including the error raised where none says what a person must check before relying on the output (`F1`, `ADR-0140`) |
| `IAcademyCatalog` | `Tempest.Core.Knowledge.Academy` | DI-public | The engineering Academy, resolving a node's children and its path from the top, stopping at a cycle rather than looping (`F2`, `ADR-0141`) |
| `IAcademyValidationService` | `Tempest.Core.Knowledge.Academy` | DI-public | Governance of the Academy, including misplaced nodes, prerequisite cycles and outcomes nothing assesses (`F2`, `ADR-0141`) |
| `IChallengeCatalog` | `Tempest.Core.Knowledge.Challenges` | DI-public | The engineering challenge library — a library, not a tutor: nothing here evaluates or scores a response (`F3`, `ADR-0141`) |
| `IChallengeValidationService` | `Tempest.Core.Knowledge.Challenges` | DI-public | Governance of challenges, including an open-ended challenge whose guidance admits no alternative answer (`F3`, `ADR-0141`) |
| `ILessonCatalog` | `Tempest.Core.Knowledge.Lessons` | DI-public | The failure and lessons database, answering what somebody should know before starting a kind of work, most serious first (`F4`, `ADR-0141`) |
| `ILessonValidationService` | `Tempest.Core.Knowledge.Lessons` | DI-public | Governance of lessons, including the error raised where an incident carries no transferable lesson, and the disclosure checks on shareable lessons (`F4`, `ADR-0141`) |
| `IWorkedExampleCatalog` | `Tempest.Core.Knowledge.WorkedExamples` | DI-public | The worked-example library, offering instructive examples ahead of merely complete ones (`F5`, `ADR-0141`) |
| `IWorkedExampleValidationService` | `Tempest.Core.Knowledge.WorkedExamples` | DI-public | Governance of worked examples, including the error raised where a quantity carries no unit and is not declared dimensionless (`F5`, `ADR-0141`) |
| `IOrganisationCatalog` | `Tempest.Core.BusinessOperations.Crm` | DI-public | The organisations the business deals with — the record `P07`'s opportunities had nothing to point at (`WP04.1`, `ADR-0142`) |
| `IContactCatalog` | `Tempest.Core.BusinessOperations.Crm` | DI-public | The people at those organisations, primary contact first and inactive last (`WP04.1`, `ADR-0142`) |
| `IInteractionCatalog` | `Tempest.Core.BusinessOperations.Crm` | DI-public | What has passed between the business and those organisations, and every agreed action nobody has done (`WP04.1`, `ADR-0142`) |
| `IOrganisationValidationService` | `Tempest.Core.BusinessOperations.Crm` | DI-public | Governance of organisations, including the error raised where one is declined without a reason (`WP04.1`, `ADR-0142`) |
| `ICrmValidationService` | `Tempest.Core.BusinessOperations.Crm` | DI-public | Governance of contacts and interactions together, because each is only sound relative to the other (`WP04.1`, `ADR-0142`) |
| `IBudgetCatalog` | `Tempest.Core.BusinessOperations.Finance` | DI-public | What the business has set aside, and which budgets are in force on a stated date (`WP04.3`, `ADR-0142`) |
| `IFinancialEntryCatalog` | `Tempest.Core.BusinessOperations.Finance` | DI-public | Commitments, accruals and actuals recorded against those budgets, including the entries allocated to none (`WP04.3`, `ADR-0142`) |
| `IBudgetValidationService` | `Tempest.Core.BusinessOperations.Finance` | DI-public | Governance of budgets, including the error raised where nobody with authority set one (`WP04.3`, `ADR-0142`) |
| `IBudgetPositionService` | `Tempest.Core.BusinessOperations.Finance` | DI-public | What was allowed, promised, paid and left — measured against commitments rather than payments (`WP04.3`, `ADR-0142`) |
| `IPurchaseRequisitionCatalog` | `Tempest.Core.BusinessOperations.Purchasing` | DI-public | What the business has asked to buy, and what is still awaiting somebody (`WP04.4`, `ADR-0142`) |
| `IPurchaseRequisitionValidationService` | `Tempest.Core.BusinessOperations.Purchasing` | DI-public | Governance of requisitions, including one recorded as ordered that names no order (`WP04.4`, `ADR-0142`) |
| `IPurchaseOrderCatalog` | `Tempest.Core.BusinessOperations.Purchasing` | DI-public | What the business has ordered, and what has not arrived (`WP04.4`, `ADR-0142`) |
| `IPurchaseOrderValidationService` | `Tempest.Core.BusinessOperations.Purchasing` | DI-public | Governance of orders, including the error raised where one is placed with nobody named as having placed it (`WP04.4`, `ADR-0135`, `ADR-0142`) |
| `INonConformanceCatalog` | `Tempest.Core.BusinessOperations.Quality` | DI-public | What did not conform, and the records closed while the problem still stands (`WP04.5`, `ADR-0142`) |
| `INonConformanceValidationService` | `Tempest.Core.BusinessOperations.Quality` | DI-public | Governance of non-conformances, including the errors raised for an unjustified concession and a record closed unresolved (`WP04.5`, `ADR-0142`) |
| `IBusinessRecordCatalog` | `Tempest.Core.BusinessOperations.Records` | DI-public | The business records the organisation keeps, and those past the retention date it decided (`WP04.6`, `ADR-0142`) |
| `IBusinessRecordValidationService` | `Tempest.Core.BusinessOperations.Records` | DI-public | Governance of business records; the platform holds no retention law and reports where the organisation has decided none (`WP04.6`, `ADR-0142`) |
...
| `IReferenceSeed<TDefinition>` | `Tempest.Core.ReferenceData.Seeding` | Platform API (contract, implemented per dataset, not itself DI-registered) | A named, versioned set of records one reference library can be populated from. Reads nothing and parses nothing: `ReferenceSeedService` writes its records through the library's own ordinary catalogue (Population phase)
...
**Total: 312 public interfaces under `src/Tempest.Core/` (311 distinct names — `IRequirement` is declared in two namespaces) — re-derived at the `v0.16.0` pre-release integration build (2026-09-07) directly against source and against this table's own rows: `grep -rhoE '^public interface ...' src/Tempest.Core` yields 312 declarations and 311 distinct names, and the Entries tables hold 312 rows. **Correction, integration build:** this line read "311 (310 distinct)", an off-by-one that subtracted the `IRequirement` duplicate from the total as well as from the distinct count. Health Check 9 could not catch it: it compares name *sets* and never reads this line. Historic narrative follows — re-derived at the Population phase (2026-09-07) by the Interface Register check in `scripts/governance-healthcheck.ps1`. The population phase adds exactly one: `IReferenceSeed<TDefinition>`. `ReferenceSeedService` is a concrete class with no interface of its own, deliberately — there is one way to seed and nothing to substitute. Previously 310 public interfaces (309 distinct names) — re-derived at `P04` (2026-09-06) by the Interface Register check in `scripts/governance-healthcheck.ps1`. `P04` adds seventeen. `IProjectDependencyRegister` is not among them: it lives in `Tempest.App`, which this register's declared scope does not cover. Previously 293 public interfaces (292 distinct names) — re-derived at `Group F` (2026-09-06) by the Interface Register check in `scripts/governance-healthcheck.ps1`. `Group F` adds ten, two per work package. Previously 283 public interfaces (282 distinct names) — re-derived at `Group E` (2026-09-06) by the Interface Register check in `scripts/governance-healthcheck.ps1`. `Group E` adds eleven: two each for `E1`, `E2`, `E4` and `E5`, and three for `E3`. `IReferencePinResolver` moved namespace from `CommercialIntelligence.Estimating` to `ReferenceData` and keeps its single row. Previously 272 public interfaces (271 distinct names) — re-derived at `Group D` (2026-09-06) by the Interface Register check in `scripts/governance-healthcheck.ps1`, which compares every declared interface name against this table's own rows. `Group D` adds twenty: three for `D1`, two each for `D2` and `D3`, eight for `D4` and five for `D5`. No existing row changed. Previously 252 public interfaces (251 distinct names) — re-derived at `Group C` (2026-09-06) by the Interface Register check in `scripts/governance-healthcheck.ps1`, which reported 252 declared against 225 rows. `Group C` adds the 27 rows the gap named: the eleven `P07` catalogues, their eleven validation services and the five reasoning services (`IContractService`, `IRiskAndInsuranceService`, `IPricingService`, `IFinancialControlService`, `IPipelineService`) — the whole of `P07`'s public surface. Historic narrative, stated when the total was 225: re-derived at `Group B` (2026-09-06) against the repository itself, by the Interface Register check in `scripts/governance-healthcheck.ps1`, which reported 225 declared against 211 rows. `Group B` adds the 14 rows the gap named: `IRuleCatalog`, `IRuleValidationService`, `IAssessmentSubject`, `IDecisionTreeCatalog`, `IDecisionTreeValidationService`, `IManufacturingDecisionService`, `IMaterialSelectionService`, `IDesignRuleService`, `IReviewDefinitionCatalog`, `IReviewDefinitionValidationService`, `IEngineeringReviewService`, `ITradeStudyCatalog`, `ITradeStudyValidationService` and `ITradeStudyService` — the whole of `P02`'s public surface. Historic narrative, stated when the total was 195: corrected at the `WP 16.4B-R2` integration (2026-09-05): `IAttachmentWriteIntentStore` added (`TD-97`); corrected at the `WP 16.4B` integration (2026-09-05): `IAttachmentContentReconciliationService`, `IMaterialCatalogReconciliationService`, `IRequirementsReconciliationService` added (`TD-67`, `TD-97`); corrected at the `WP 16.3B` integration (2026-09-04): `IStateMigration`, `IStateMigrationRegistry`, `ISettingsMigration<TDocument>` added (`ADR-0120`); previously corrected
