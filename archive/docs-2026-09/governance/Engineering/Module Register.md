...
## Fault-Injection Modules (`Tempest.Validation`, WP 12.3B, ADR-0102)

Real, shipped, tested modules — not test fixtures (see "Test-Only Module
Fixtures," below, for the actual out-of-scope category) — that exist
solely to deliberately fail, for validation purposes, never a genuine
application capability. Excluded from every ordinary Discovery run by
default (`IFaultInjectionModule`); never referenced by `Tempest.App`/
`Tempest.Desktop`; explicitly enable-able via
`ITempestHostBuilder.EnableFaultInjectionModules()`. Counted separately
from the 34 production-module total above, which reflects only what
`ClockModuleDiscoveryTests`'s own assembly-scoped scan of `Tempest.Samples`
finds (33, as of this Work Package) plus this one fault-injection module,
34 in total, unchanged.

| Module | Namespace | Base Type | Uses `ModuleMetadataAttribute` | Constructor-Injects | Originating Work Package |
|---|---|---|---|---|---|
| `DuplicateNavigationModule` | `Tempest.Validation.FaultInjection` | `ModuleLifecycleBase`, `IFaultInjectionModule` | Yes | `INavigationProvider` | WP 5.0B (created, as `DuplicateNavigationSampleModule`), WP 12.3B (moved from `Tempest.Samples`, renamed, ADR-0102) |
