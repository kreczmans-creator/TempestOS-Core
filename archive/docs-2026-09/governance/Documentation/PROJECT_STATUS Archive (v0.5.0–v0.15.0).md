# PROJECT_STATUS Archive (v0.5.0–v0.15.0)

## What This Document Is

This is the **verbatim retained content** of `PROJECT_STATUS.md` from
the line `## READY FOR WP14 UI/UX` to the end of the file, as it stood
immediately before `WP 16.2A` (Register and Status Currency,
`v0.16.0`, 2026-09-04) rewrote `PROJECT_STATUS.md` as a short, current,
live document. It is **retained, not deleted** — per `PROJECT_STATUS.md`'s
own long-standing convention of disclosing corrections and supersessions
rather than silently erasing them, this archive *is* that disclosure,
applied once to the file's own accumulated history rather than to a
single paragraph.

**Why this exists.** `PROJECT_STATUS.md` had grown to roughly 9,068
lines (565 KB) — eleven releases of "this field is stale, corrected
below" narration layered on top of itself, most of it superseded by a
later paragraph earlier in the same file, none of it ever removed. That
made the live document unusable as the "read this first" dashboard it
is meant to be, and `PROJECT_STATUS.md`'s own size worked directly
against `docs/releases/v0.16.0/v0.16.0 Release Plan.md`'s `WP 16.2A`
acceptance criterion that the rewritten file stay well under 60 KB.
This archive is the other half of that split: nothing here is
discarded, only moved and clearly labelled.

**Where the live equivalents now are.** Everything of current,
standing value that was buried in this archived text has a live,
current home elsewhere:

- **Current release, branch, and Work Package status** — the top of
  `PROJECT_STATUS.md` itself (rewritten short).
- **Repository Metrics** — `docs/governance/Quality/Repository Metrics
  Register.md`'s own `WP 16.2A` snapshot.
- **Repository Health / build / test facts** — `docs/governance/Quality/
  Validation Register.md`'s own `WP 16.2A` "Current State" section.
- **Documentation Status** — `docs/governance/Documentation/
  Documentation Register.md`.
- **Academy Status** — `docs/governance/Documentation/Academy
  Register.md`.
- **Governance Status** — `docs/governance/Governance Index.md` and
  `docs/governance/Governance Audit Report.md`.
- **Known Unknowns** — carried forward verbatim into the rewritten
  `PROJECT_STATUS.md`'s own "Known Unknowns" section.
- **Next Planned Work Package / Near-Term Roadmap** — `docs/releases/
  v0.16.0/WorkPackages.md` and `docs/releases/v0.16.0/v0.16.0 Release
  Plan.md`.
- **Long-Term Vision** — `VISION.md` and `docs/governance/Product
  Roadmap.md`.
- **Every register-level count** (Interfaces, Exceptions, DI
  registrations, Namespaces, Platform Services, Feature/Release
  history) — the individual `docs/governance/Engineering/**` and
  `docs/governance/Delivery/**` registers, each re-derived directly by
  `WP 16.2A` and no longer duplicated here.

**A disclosed boundary correction.** `docs/releases/v0.16.0/v0.16.0
Release Plan.md`'s own `WP 16.2A` scope describes the archive as
holding what moves "from `Current Repository Phase`" (the section that,
in the pre-`WP 16.2A` file, began partway through this same document).
This archive instead begins earlier, at `## READY FOR WP14 UI/UX` —
because that section and everything between it and `Current Repository
Phase` is itself stale, `v0.13`-era content (a plugin-platform
readiness review predating `v0.14.0`, `v0.15.0`, and this Work
Package's own base entirely) that the plan's literal boundary would
have left in neither the archive nor the rewritten live file — genuinely
lost, not merely relocated. Archiving from `## READY FOR WP14 UI/UX`
instead closes that gap: every line between the old file's own top
matter and its end is now accounted for, either in this archive or
folded, in substance, into the rewritten `PROJECT_STATUS.md`'s own
top-of-file status paragraphs. This is a deliberate widening of the
plan's own literal instruction, disclosed here rather than silently
applied.

**What is not here.** The original file's own top-of-file status
paragraphs (roughly its first 1,023 lines — the `v0.14.0`/`v0.15.0`/
`WP 15.2A` release-status narration) are not reproduced verbatim in
this archive; they were superseded in place by the rewritten
`PROJECT_STATUS.md`'s own new top section, which carries their
substance forward directly (current release, current branch, current
Work Package) rather than as a second, parallel historical copy.
