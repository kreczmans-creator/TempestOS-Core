...
| **Module Discovery** *(existing, `IFrameworkDiscoveryService`)* | Scans **all** loaded assemblies — including plugin assemblies Plugin Loading just loaded — for `IModule` types, exactly as today. **Corrected, `WP 13.12.2`:** no longer literally unchanged — `WP 13.9.6` added an optional, generic `Func<Type, bool>? isTypeExcluded` predicate, and `WP 13.11B` a fail-closed reflection guard. The accurate claim is **plugin-unaware**: neither addition references any `Tempest.Core.Plugins` type. | ~~**Unchanged**~~ **Plugin-unaware** |
| **Module Registration** *(existing, `RuntimeModuleManager`)* | Registers whatever descriptors Module Discovery finds. Has no plugin-specific logic at all — still true of `RuntimeModuleManager` itself; **corrected, `WP 13.12.2`:** `WP 13.9.4` added a trust-denial filter in `TempestHost`'s own orchestration immediately before registration. | ~~**Unchanged**~~ **Plugin-unaware** |
| **Module Lifecycle** *(existing, `ModuleLifecycleManager`)* | Drives registered modules through initialisation, startup, shutdown, disposal. **Corrected, `WP 13.12.2`:** a module arriving via a plugin is no longer wholly indistinguishable — `WP 13.2A` added an optional `componentScopeProvider` hook so a plugin module's lifecycle calls run under its own component principal (`ADR-0111`). The hook is a generic `Func<>`; no `Tempest.Core.Plugins` reference. | ~~**Unchanged**~~ **Plugin-unaware** |
