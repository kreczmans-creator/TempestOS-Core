...
> **Current state, and what "released tree" above does and does not mean.**
> *(Added by the `WP 16.4B-R7` governance closure, 2026-09-07, from the
> repository rather than from any report.)*
>
...
>   is the tree on `main`; it is **not** the candidate and has not been
>   since `WP 16.4B-R6`. Read as this release's current acceptance
>   evidence, that sentence is stale.
> - **The fourth matrix has been obtained** — 5/5 on `888ef63`, runs
>   253–257, ERR **§4.4** — and is **superseded**, because `WP 16.4B-R7`
>   changed three files under `src/` and five under `tests/` between
>   `888ef63` and `d6af7ec`. **A fifth matrix is outstanding, against
>   `d6af7ec`, and `d6af7ec` is not matrix-certified.** This pass did not
>   run it and does not promise it.
> - **"Every row below is delivered and validated" overstates the table.**
>   The table has rows for `WP 16.4B`, `-R3` and `-R4` only; there is no
>   row for `WP 16.4B-R1`, `-R2`, `-R5`, `-R6` or `-R7`, and
>   `WP 16.4B-R6` and `-R7` are not "delivered and validated" —
>   `-R7` is the round that produced the candidate and is recorded in no
>   document in `docs/`. The missing-rows half was raised by the sixth
>   board (`v0.16.0 Review Board Disposition.md` §8.5) and is still open;
>   writing those rows is delivery work, not a currency correction, and
>   this pass has flagged it rather than inventing them.
> - **"Engineering complete" is the status of the release's thirteen
>   planned Work Packages, not of the remediation chain**, which is still
>   running at `-R7`.
