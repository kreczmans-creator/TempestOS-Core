...
| D11 | Rail → **Business** → **Invoices** | Requests are grouped by their own status — Draft, Sending, Sent, Accepted, Rejected, Unknown, Reauthorise, Voided, in that order (an "available to invoice" item is a completed-but-unraised deliverable on its own Deliverables tab, not a group here — the PO's sketched Invoices grouping is not what this area shows). **Send** (Draft only) dispatches through the configured connector — **Fake** in review; **Xero** and **QuickBooks Online** are the other two named in Settings → Invoicing. **Reconcile now** reads external status and dates from the connector. No control anywhere sets Paid — `PaidDate` is written only from what the connector itself reports (`InvoicingView.cs`, `StatusOrder`; `SettingsView.cs`; `InvoicingService.cs`). | A grouping other than by status; a Paid control anywhere; Send/Reconcile act without going through the connector. |
...
| D15 | Rail → **Engineering** → **Tasks**, then → **Reference data** → open a library record | Tasks shows only **Reviews** and **Approvals** — the tasks read model carries no separate Calculations bucket, so that sketched sub-heading is left out rather than shown empty. Reference data lists all eight libraries (Materials, Fasteners, Bearings, Standards, Constants, Manufacturing, Components, Rate Cards); opening a record (double-click or **Open**) shows its identity, every field the definition declares rendered generically with its own unit, revision history with the current revision marked "(current)", its source citation, every Evidence record that cites it under "cited by", and **Verify** / **Release** / **Revise** sized to its own state — Release from Draft verifies first, as one action (`EngineeringAreaView.cs`; `ReferenceRecordView.cs`). | A Calculations sub-heading shown empty; a library missing; a record's fields shown as raw text with no unit. |
...
| D19 | Projects tree → a project closed 90 days or more ago | It lists under **Archive (90 days and over)** rather than **Closed (under 90 days)** (`ProjectArchival.ArchiveAfterDays = 90`); opening it shows "Archived — closed {date}. Reference data only; nothing here can be changed."; every write the review can reach on it is refused, each message naming the project and its closed date — a new quotation, an edit to an existing quotation, a commercial change, a new or existing deliverable completion, a new or existing timesheet entry, a new or existing invoice request — and **Reopen** itself is refused ("it is Archive and read-only. It cannot be reopened."). This guard sits in the commercial, quotation, deliverable, timesheet and invoicing services only; nothing in the shell offers a write on an archived project through any other surface, but the milestone/engineering-task, evidence, requirements and task services carry no structural guard of their own yet (`ProjectArchival.cs`; `ProjectsAreaView.cs`; `ProjectWorkspaceView.cs`; `QuotationService.cs`; `ProjectCommercialService.cs`; `DeliverableService.cs`; `TimesheetService.cs`; `InvoicingService.cs`; `ProjectLifecycleService.cs`). | A project closed 90+ days ago still lists as Closed; a write on it succeeds anywhere the review can reach; a refusal does not name the project or its closed date. |
