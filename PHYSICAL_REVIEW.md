...
| D2 | Rail → **Projects** → **Open** → **New Project…**; name it, leave **Open a quotation for this project** ticked (checked by default) | The project is created and opens directly on its **Quote** tab, already carrying one Draft quotation: reference generated `Q-<yyyy>-<nnn>`, client defaulted from the project, currency from its rate-card pin (GBP otherwise), validity 30 days (`NewProjectPrompt.cs`, `ProjectQuoteView.cs`, `QuotationService.CreateAsync`/`NextReferenceAsync`). | No quotation exists after creation; the project opens on Overview instead of Quote; the reference is not `Q-yyyy-nnn`-shaped. |
...
| D4 | **Export** | The save picker opens pre-named `<reference>-quote.pdf`; the saved file is a real PDF headed QUOTATION, with the issuer's name, project code and name, client, reference/date/validity, status/currency, a Lines table (Description / Hours / Rate / Amount), the Total, and Terms, plus a footer (app version, reference, generation time, page number) on every page. The Release Notes describe this sheet as a fixed one page that clips an overrun; the renderer's own test says otherwise — a short quote is one page, and one whose lines overrun a page continues onto further pages, each repeating the table's column headers and carrying its own footer, never clipping (`QuotationSheetRenderer.cs`; `tests/Tempest.Desktop.Tests/Quotations/QuotationSheetRendererTests.cs`, `Render_ProducesAValidOnePagePdf` and `Render_FortyLines_Paginates_AndEveryPageCarriesTheFooter`). | No picker; the saved file is not a valid PDF; a continuation page is missing its own footer or its table header. |
...
| D15 | Rail → **Engineering** → **Tasks**, then → **Reference data** → open a library record | Tasks shows **Calculations**, **Reviews** and **Approvals** — Calculations lists every live Calculation under a project, not yet complete and not yet cited by issued evidence, each row with an **Open** (right up) and a **Complete** action (`WP 20.1B`, `TD-181`). Reference data lists all eight libraries (Materials, Fasteners, Bearings, Standards, Constants, Manufacturing, Components, Rate cards), each with its own heading even when it holds no record yet (reading "(0)" with "No records yet" beneath it, its own Add action — where one exists — still available); opening a record (double-click or **Open**) shows its identity, every field the definition declares rendered generically with its own unit, revision history with the current revision marked "(current)", its source citation, every Evidence record that cites it under "cited by", and **Verify** / **Release** / **Revise** sized to its own state — Release from Draft verifies first, as one action (`EngineeringAreaView.cs`; `ReferenceRecordView.cs`). | A Calculations heading missing, or shown empty when a live calculation exists; Complete does not remove a row; a library missing; a record's fields shown as raw text with no unit. |
...
| D18 | Project → **Sign off**: enter a statement, **Sign off** | A blank statement is refused before any service call ("A statement is required before signing off."); signing off records who and when, closes the project, and the tab shows the record verbatim: "Signed off by {principal} on {date}, closing the project on {date}." with the statement quoted underneath, and the status line reads "Signed off; the project is now closed."; **Reopen** appears once closed, and works within 90 days of the close date; **Hold**/**Resume** are offered only while the project is open (`ProjectSignOffView.cs`; `ProjectLifecycleService.SignOffAsync`/`ReopenAsync`/`HoldAsync`/`ResumeAsync`). | A blank statement is accepted; the record does not quote the statement; Reopen is offered on an Archive-age project. |
...
| # | Step | Expected result | Counts as a failure if |
|---|---|---|---|
| T1 | Projects → a project's editor → **Commercial** → change client → **Add organisation** | The organisation form carries a **Payment terms** drop-down reading Up front / 30 days / 60 days, Up front selected by default (`src/Tempest.Desktop/Views/OrganisationPicker.cs`, `WP 20.1B`). Complete a deliverable for that client (§7c D10) and open Business → Invoices: the request's row reads "… — Terms {terms}", and once sent "… — Due {date}"; the **Outstanding / Overdue** group is unpaid past its own due date, not thirty days after sending (`src/Tempest.Desktop/Views/InvoicingView.cs`). | No drop-down; a request raised for a 60-day client shows no terms or a due date thirty days out; changing the client's terms afterwards changes an already-raised request. |
| T2 | Project → **Structure** → Calculations → create a calculation; then rail → **Engineering** → **Tasks** | The calculation lists under **Calculations** the moment it exists — a task from creation, with **Open** and **Complete** on its row; **Complete** removes it (`src/Tempest.Desktop/Views/EngineeringAreaView.cs`; `TD-181`, `WP 20.1B`). Rail → **Tasks** shows the same bucket. Issuing evidence that cites the calculation removes it too. | The calculation appears only after a restart; Complete needs a second person; a calculation with no project appears as a task. |
...
| T4 | Structure → dock the Requirements tree beside a requirement's editor; edit the requirement's title | The tree's row updates without re-entering the tab (`src/Tempest.Core/Requirements/RequirementsService.cs` publishes every write on the change bus; `TD-28`, `WP 20.1A1`). | The tree still shows the old title until the tab is left and re-entered. |
| T5 | Project → Documents → attach a `.dwg` (any file renamed will do) and a PDF; **Open** each | The DWG's viewer says "This drawing opens in its own application" with an **Open externally** button, and nothing else pretends to render it; the PDF renders, and the toolbar's ⟲ / ⟳ rotate it in 90° steps (`src/Tempest.Desktop/Viewing/DocumentViewerView.cs`; `WP 20.2B`). An `.svg` is reported unsupported — Release Notes Warning. | A blank page with no message; a rotated page never refits; Open externally does nothing. |
| T6 | Structure → select an object in the Project Explorer → **Ctrl+Shift+M**; then **Ctrl+Shift+C** | The object picker opens — objects by Kind, a **Filter…** box, the open project's own objects first, a top-level row — and **Choose** moves the object under the chosen parent; Ctrl+Shift+C copies it there; both report in the status bar and land in command history (`src/Tempest.Desktop/Views/ObjectPickerDialog.cs`, `src/Tempest.Desktop/Composition/MainWindowComposer.Wire.cs`; `S2-2`, `WP 20.2A`). Choosing the object's own descendant is refused with the same message drag-and-drop gives. | No dialog; the shortcut moves the wrong object; Copy still says it is unavailable. |
