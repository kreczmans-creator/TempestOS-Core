...
**2.3 Core tests** — 4,949 tests, ~20–50 seconds (re-derived at `v0.17.0` after `WP 17.9.2`, 2026-09-08; the `v0.16.0` tree had 5,153 before WP 17.0C removed scaffolding and WP 17.2A froze the plugin, REST and licensing suites)

```
dotnet test tests/Tempest.Core.Tests/Tempest.Core.Tests.csproj --configuration Debug --no-build
```

**2.4 Desktop tests** — 501 tests, ~2.5–3 minutes (re-derived at `v0.17.0` after `WP 17.9.2`, 2026-09-08; set `TEMPEST_TEST_TIMEOUT_FACTOR=3` when the machine is busy)
...
| 6 | Ribbon → **Mechanical** → a **Create** action, with nothing selected | A prompt collects the values the command declares (name, and a Kind where the command offers one). The status bar says *Created Part '…'. It is under '<your project>' in the Project Explorer.* and the new object appears **under the open project**, selected (`WP 17.9.2`). Select an Assembly first and create again: the new object goes under that Assembly. | Nothing is created; the prompt collects nothing; the message names a Guid; the object is not under the project or is nowhere in the Explorer. |
